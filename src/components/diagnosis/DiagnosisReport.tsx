...
import { Activity, Utensils, AlertCircle, HeartPulse, Leaf, Info, Download, Check, User, Stethoscope, Pill, MapPin, Clock, Brain, Moon, Dumbbell, AlertTriangle, Calendar, QrCode, PenTool, MessageCircle, Image as ImageIcon, Share2 } from 'lucide-react'
...
📝 *${t.detailedAnalysis}:*
${analysisText.substring(0, 1500)}${analysisText.length > 1500 ? '...' : ''}

${getFoodRecommendations().length > 0 ? `🥗 *${t.recommendedFoods}*
${getFoodRecommendations().map((f: string) => `• ${f}`).join('\n')}` : ''}

${getFoodsToAvoid().length > 0 ? `🚫 *${t.foodsToAvoid}*
${getFoodsToAvoid().map((f: string) => `• ${f}`).join('\n')}` : ''}

${data.recommendations?.lifestyle?.length ? `🌿 *${t.lifestyleRecommendations}*
${data.recommendations.lifestyle.map((tip: string) => `• ${tip}`).join('\n')}` : ''}

${data.recommendations?.exercise?.length ? `🏃 *${t.exercise}*
${data.recommendations.exercise.map((ex: string) => `• ${ex}`).join('\n')}` : ''}

${data.recommendations?.acupoints?.length ? `📍 *${t.acupoints}*
${data.recommendations.acupoints.map((pt: string) => `• ${pt}`).join('\n')}` : ''}

━━━━━━━━━━━━━━━━━
🌿 Generated by Sihat TCM AI
📱 https://sihat-tcm.vercel.app`
        ...
        if (navigator.share) {
            try {
                ...
                // Try to create and share PDF file
                try {
                    const doc = new jsPDF()
                    const pageWidth = doc.internal.pageSize.getWidth()
                    const margin = 20
                    ...
                    doc.setTextColor(68, 64, 60)
                    doc.setFontSize(12)
                    doc.text(`${t.mainDiagnosis}: ${diagnosisText}`, margin, 40)
                    doc.text(`${t.constitution}: ${constitutionText}`, margin, 55)

                    // Convert to blob and file
                    const pdfBlob = doc.output('blob')
                    const isoDate = new Date().toISOString().split('T')[0]
                    const pdfFile = new File([pdfBlob], `${t.fileName}_${isoDate}.pdf`, { type: 'application/pdf' })
    ...
    const fallbackShare = async (text: string) => {
        try {
            ...
            alert(language === 'zh' ? '报告已复制到剪贴板！您可以粘贴到任何应用程序中分享。' :
                language === 'ms' ? 'Laporan telah disalin ke papan klip! Anda boleh tampal ke mana-mana aplikasi untuk berkongsi.' :
                    'Report copied to clipboard! You can paste it into any app to share.')
        } catch (error) {
            console.error('Failed to copy:', error)
            alert(language === 'zh' ? '无法复制报告。请手动复制。' :
                language === 'ms' ? 'Tidak dapat menyalin laporan. Sila salin secara manual.' :
                    'Unable to copy report. Please copy manually.')
        }
    }


    // Condensed mobile report summary for chat mode
            ...
            <div className="max-w-lg mx-auto space-y-3">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h3 className="font-semibold text-emerald-800 text-sm">TCM Report Summary</h3>
                            <p className="text-xs text-emerald-600">Reference while chatting</p>
                        </div>
                    </div>
                    <button
                ...
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="bg-white/60 rounded-lg p-3 border border-emerald-100">
                        <p className="text-xs text-emerald-600 font-medium mb-1">Diagnosis</p>
                        <p className="text-sm font-semibold text-emerald-800 line-clamp-2">{diagnosisText}</p>
                    </div>

                    {/* Constitution */}
                    <div className="bg-white/60 rounded-lg p-3 border border-emerald-100">
                        <p className="text-xs text-emerald-600 font-medium mb-1">Constitution</p>
                ...
                <div className="flex flex-wrap gap-1.5">
                    ...
                    {getFoodRecommendations().length > 0 && (
                        <span className="text-xs px-2 py-1 bg-white/60 rounded-full text-stone-600 border border-stone-200">
                            🥗 {getFoodRecommendations().length} foods
                        </span>
                    )}
                    {data.recommendations?.lifestyle?.length && (
                        <span className="text-xs px-2 py-1 bg-white/60 rounded-full text-stone-600 border border-stone-200">
                            🌿 {data.recommendations.lifestyle.length} lifestyle tips
                        </span>
                    )}
                </div>
            </div>
    ...
    const content = (
        <div className={`transition-all duration-300 ${isChatExpanded ? 'fixed inset-0 z-50 bg-stone-50 flex flex-col md:flex-row overflow-hidden' : ''}`}>
            {/* Mobile: Show condensed report summary on top when chat is expanded */}
            {isChatExpanded && (
            ...
            {/* Left Panel (Desktop) / Hidden on Mobile when expanded - Report Content */}
            <motion.div
                variants={container}
                initial="hidden"
                animate="show"
                className={`space-y-4 md:space-y-6 max-w-4xl mx-auto px-2 md:px-0 ${isChatExpanded ? 'hidden md:block md:w-1/2 h-full overflow-y-auto p-6 border-r border-stone-200' : ''}`}
            >
                {/* Header Section */}
            ...
            {/* Right Panel (Desktop) / Bottom Panel (Mobile) - Chat Interface (Only visible when expanded) */}
            {isChatExpanded && (
