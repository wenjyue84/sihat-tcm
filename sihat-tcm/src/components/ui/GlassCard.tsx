...
function GlassCard({
  ...
  // Intensity styles for backdrop blur and background (Apple-inspired)
  const intensityStyles = {
    low: "backdrop-blur-[10px] bg-white/30 dark:bg-white/5",
    medium: "backdrop-blur-[20px] bg-white/50 dark:bg-white/10",
    high: "backdrop-blur-[40px] bg-white/70 dark:bg-white/20",
  };

  // Animation variants (Apple-inspired easing)
  const motionVariants = {
    ...
    visible: {
      ...
      transition: {
        duration: 0.3,
        ease: [0.25, 0.1, 0.25, 1] as any, // Apple's standard easing
      },
    },
  };
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "relative rounded-[18px] border p-6 transition-apple-normal shadow-depth-2",
        // Blur and background
        intensityStyles[intensity],
        // Border and shadow
        variantStyles[variant],
        // Responsive interaction states (Apple-inspired)
        "md:hover:-translate-y-0.5 md:hover:brightness-[1.02] md:hover:shadow-depth-3 active:scale-[0.99]",
        // Glow effect wrapper
