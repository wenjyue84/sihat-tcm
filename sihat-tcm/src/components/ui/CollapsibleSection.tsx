...
export function CollapsibleSection({
  ...
  const accentClass =
    colorClasses[accentColor as keyof typeof colorClasses] || colorClasses.emerald;
  const iconClass =
    iconColorClasses[accentColor as keyof typeof iconColorClasses] || iconColorClasses.emerald;
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-base md:text-lg font-semibold text-stone-800 text-left">{title}</h3>
        </div>
        ...
        {isOpen && (
          ...
          >
            <div className="p-3 md:p-5 pt-0">
              <div className="pt-4 border-t border-stone-100">{children}</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
