...
export function WeeklyCalendarView({ mealPlan, onRefresh }: WeeklyCalendarViewProps) {
  ...
  const handleSwap = async (mealType: string, currentMeal: any) => {
    const res = await swapMeal(mealPlan.id, selectedDay - 1, mealType as any, currentMeal);
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold mb-2">
              {t.patientDashboard.mealPlanner.yourMealPlan}
            </h2>
            <p className="text-amber-100">
              {t.patientDashboard.mealPlanner.constitution}:{" "}
              {translateConstitution(mealPlan.constitution, t)} • {completedDays.length}/7{" "}
              {t.patientDashboard.mealPlanner.daysCompleted}
            </p>
          </div>
      ...
      {planData.tcm_principles && (
        <Card className="p-6 bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-200">
          ...
          <p className="text-emerald-800 leading-relaxed">{planData.tcm_principles}</p>
        </Card>
      )}
        ...
        {weeklyPlan.map((day: any) => (
          ...
          >
            <div className="text-xs opacity-80 mb-1">
              {t.patientDashboard.mealPlanner.day} {day.day}
            </div>
            <div className="text-sm font-bold">
              {day.theme || `${t.patientDashboard.mealPlanner.day} ${day.day}`}
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              >
                ...
                {completedDays.includes(selectedDay)
                  ? t.patientDashboard.mealPlanner.completed
                  : t.patientDashboard.mealPlanner.markComplete}
              </Button>
            </div>
