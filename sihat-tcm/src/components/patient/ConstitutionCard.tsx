...
const constitutionColors: Record<string, { from: string; to: string; accent: string }> = {
    ...
    'Special Constitution': { from: 'from-violet-500', to: 'to-fuchsia-500', accent: 'text-violet-700' },
...
function extractKeyCharacteristics(constitutionData: ConstitutionCardProps['latestConstitutionData']): string[] {
    const characteristics: string[] = []

    if (!constitutionData) return characteristics

    // Try to extract from lifestyle recommendations
    if (constitutionData.recommendations?.lifestyle && constitutionData.recommendations.lifestyle.length > 0) {
        // Take first 2-3 items and simplify them
        characteristics.push(...constitutionData.recommendations.lifestyle.slice(0, 3).map(item => {
...
export function ConstitutionCard({ latestConstitution, latestConstitutionData }: ConstitutionCardProps) {
                    ...
                    <div className="relative p-8 flex flex-col md:flex-row items-center justify-between gap-6 text-white">
                        <div className="flex-1 text-center md:text-left">
                            ...
                            <h2 className="text-2xl md:text-3xl font-black mb-3 tracking-tight">
                                {t.patientDashboard.constitutionCard?.noDiagnosisTitle || 'Discover Your TCM Constitution'}
                            </h2>
                            <p className="text-white/90 text-sm md:text-base max-w-2xl">
                                {t.patientDashboard.constitutionCard?.noDiagnosisDesc || 'Start your TCM diagnosis journey to understand your unique body constitution and receive personalized health recommendations.'}
                            </p>
                        </div>
                        <Button
                            onClick={() => router.push('/')}
                        ...
                        >
                            {t.patientDashboard.constitutionCard?.startAssessment || 'Start Constitution Assessment'}
                        </Button>
                    </div>
                </Card>
            </motion.div>
                            ...
                            >
                                <div className="grid md:grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            {latestConstitutionData.recommendations.dietary_advice.foods_to_eat && (
                                                <div className="mb-4">
                                                    ...
                                                    <ul className="text-sm space-y-1 opacity-80">
                                                        {latestConstitutionData.recommendations.dietary_advice.foods_to_eat.slice(0, 4).map((food, index) => (
                                            ...
                                            {latestConstitutionData.recommendations.dietary_advice.foods_to_avoid && (
                                                <div>
                                                    ...
                                                    <ul className="text-sm space-y-1 opacity-80">
                                                        {latestConstitutionData.recommendations.dietary_advice.foods_to_avoid.slice(0, 4).map((food, index) => (
                                                            <li key={index}>• {food}</li>
                                                        ))}
                                                    </ul>
                                                </div>
                                    ...
                                    {latestConstitutionData.recommendations?.lifestyle && (
                                        <div>
                                            <h3 className="font-bold text-lg mb-3">
                                                {t.patientDashboard.constitutionCard?.lifestyleAdvice || 'Lifestyle Recommendations'}
                                            </h3>
                                            <ul className="text-sm space-y-2 opacity-90">
                                                {latestConstitutionData.recommendations.lifestyle.slice(0, 5).map((tip, index) => (
                                ...
                                {latestConstitutionData.created_at && (
                                    <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                                        <p className="text-sm opacity-75">
                                            {t.patientDashboard.constitutionCard?.lastAssessed || 'Last assessed'}: {new Date(latestConstitutionData.created_at).toLocaleDateString()}
                                        </p>
                                        <Button
                                            variant="outline"
                                            onClick={() => router.push('/')}
                                        ...
                                        >
                                            {t.patientDashboard.constitutionCard?.getNewAssessment || 'Get New Assessment'}
                                        </Button>
                                    </div>
                                )}
                            </motion.div>
