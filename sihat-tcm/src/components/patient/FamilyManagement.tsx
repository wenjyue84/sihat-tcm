...
const MOCK_FAMILY_MEMBERS: FamilyMember[] = [
    {
        ...
        lastDiagnosis: {
            date: '2024-12-20',
            diagnosis: 'Qi Deficiency with Dampness',
            score: 72
        }
    },
    {
        ...
        lastDiagnosis: {
            date: '2024-12-18',
            diagnosis: 'Yang Deficiency',
            score: 68
        }
    },
    {
        id: '3',
        name: 'Mei Ling (Daughter)',
        age: 8,
        gender: 'female',
        relationship: 'child'
    }
...
export function FamilyManagement() {
    ...
    const handleAddMember = async () => {
        ...
        setFormData({ name: '', age: '', gender: '', relationship: '', medicalHistory: '' })
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm text-slate-600">
                                {(t.familyManagement?.membersCount || '{count} family members').replace('{count}', familyMembers.length.toString())}
                            </p>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-slate-500 capitalize">
                                                        {t.familyManagement?.relationships?.[member.relationship] || member.relationship}
                                                        {' • '}
                                        ...
                                        {member.lastDiagnosis ? (
                                            <div className="space-y-2">
                                                <div className="flex items-center gap-2 text-xs text-slate-500">
                                                    ...
                                                    {t.familyManagement?.lastDiagnosis || 'Last Diagnosis'}: {formatDate(member.lastDiagnosis.date)}
                                                </div>
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    {member.lastDiagnosis.score && (
                                                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${getScoreBadge(member.lastDiagnosis.score)?.bg} ${getScoreBadge(member.lastDiagnosis.score)?.text}`}>
                                                            {member.lastDiagnosis.score}
                                                        </span>
                                                    )}
                                                </div>
                    ...
                    >
                        <CardContent className="p-6 flex flex-col items-center justify-center h-full text-center">
                            ...
                            <p className="text-xs text-slate-400 mt-1">
                                Create a health profile for a loved one
                            </p>
                        </CardContent>
                    </Card>
                </motion.div>
            ...
            <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Create a health profile for your loved one. Their data will be tracked separately under your account.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="name">
                                {t.familyManagement?.addForm?.name || 'Full Name'} <span className="text-red-500">*</span>
                            </Label>
                            <Input
                                id="name"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <Label htmlFor="age">
                                    {t.familyManagement?.addForm?.age || 'Age'} <span className="text-red-500">*</span>
                                </Label>
                                <Input
                                    id="age"
                                    type="number"
                            ...
                            <div className="space-y-2">
                                <Label>
                                    {t.familyManagement?.addForm?.gender || 'Gender'} <span className="text-red-500">*</span>
                                </Label>
                                <Select
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        <SelectItem value="male">{t.patientDashboard?.profile?.male || 'Male'}</SelectItem>
                                        <SelectItem value="female">{t.patientDashboard?.profile?.female || 'Female'}</SelectItem>
                                        <SelectItem value="other">{t.patientDashboard?.profile?.other || 'Other'}</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            <Label>
                                {t.familyManagement?.addForm?.relationship || 'Relationship'} <span className="text-red-500">*</span>
                            </Label>
                            <Select
                                value={formData.relationship}
                                onValueChange={(value: FamilyMember['relationship']) => setFormData({ ...formData, relationship: value })}
                            >
                                <SelectTrigger>
                    ...
                    <DialogFooter className="gap-2">
                        ...
                        <Button
                            ...
                            disabled={isSubmitting || !formData.name || !formData.age || !formData.gender || !formData.relationship}
