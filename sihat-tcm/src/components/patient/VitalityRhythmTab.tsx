...
export function VitalityRhythmTab({ sessions }: VitalityRhythmTabProps) {
  ...
  const getElementType = (element: string): "Wood" | "Fire" | "Earth" | "Metal" | "Water" => {
    ...
    if (elementLower === "earth" || elementLower === "土" || elementLower === "tanah")
      return "Earth";
    if (elementLower === "metal" || elementLower === "金" || elementLower === "logam")
      return "Metal";
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-none shadow-xl bg-white overflow-hidden rounded-3xl">
            <CardHeader className="bg-gradient-to-r from-indigo-50 to-blue-50/30 border-b border-slate-100">
              ...
              <CardDescription>{vitalityT.meridianClockDesc}</CardDescription>
            </CardHeader>
                  ...
                  {[...Array(12)].map((_, i) => (
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor:
                          i === Math.floor(currentTime.getHours() / 2) % 12 ? "#4f46e5" : "#e2e8f0",
                      }}
                    />
                  ))}
                </div>
                    ...
                    <div className="relative z-10">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-bold text-indigo-400 uppercase tracking-widest">
                          {vitalityT.activeOrgan}
                        </span>
                        <span className="text-xs font-bold text-indigo-600 bg-white px-2 py-1 rounded-md shadow-sm">
                          {currentOrgan.time}
                        </span>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">
                        {vitalityT.comingNext}
                      </p>
                      <p className="font-bold text-slate-700">
                        {getActiveOrgan((currentTime.getHours() + 2) % 24).name}
                      </p>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">
                      ...
                      <p className="font-bold text-slate-700">
                        {getActiveOrgan((currentTime.getHours() - 2 + 24) % 24).name}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6 flex-1">
                  <div>
                    ...
                    <div className="p-5 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10">
                      <div className="flex gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-indigo-200 text-sm mb-1 uppercase">
                            {vitalityT.healthWisdom}
                          </h4>
                          <p className="text-indigo-50/80 text-sm leading-relaxed">
                            {vitalityT.healthWisdomDesc ||
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 text-white/60">
                      ...
                      <span>
                        {vitalityT.comingNext}: <strong>{solarTerm.next}</strong>
                      </span>
                    </div>
        ...
        <div className="space-y-6">
          <Card className="border-none shadow-lg bg-white rounded-3xl overflow-hidden">
            ...
            <CardContent className="p-6">
              <p className="text-sm text-slate-500 mb-6">{vitalityT.constitutionTrackerDesc}</p>
                        ...
                        {entry.characteristics && (
                          <div className="mt-2 flex flex-wrap gap-1">
                            {entry.characteristics.split(",").map((char, index) => (
                              <span
                                key={index}
                                className="text-[9px] px-2 py-0.5 bg-slate-50 text-slate-500 rounded-full border border-slate-100"
                              >
                                {char.trim()}
                              </span>
                            ))}
          ...
          <Card className="border-none shadow-lg bg-white rounded-3xl overflow-hidden">
            ...
            <CardContent className="p-6">
              ...
              <p
                ...
                dangerouslySetInnerHTML={{ __html: vitalityT.didYouKnowDesc }}
              />
            </CardContent>
          </Card>
        </div>
