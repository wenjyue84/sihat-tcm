...
// SpeechRecognition types are available globally via src/types/speech.d.ts
...
`
    ...
    if (!hasRequestedInitialQuestion.current && messages.length === 0) {
      ...
      if (uploadedFiles.length > 0 && !hasInformedAboutFiles.current) {
        ...
        const fileSummary = uploadedFiles
          .map((f) => `${f.name} (${f.type}): ${f.extractedText ? "Text extracted" : "Image only"}`)
          .join(", ");
        const fileContext =
          language === "zh"
  ...
  const startVoiceRecognition = useCallback(() => {
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      const msg =
        errorMessages[errorKey]?.[language] ||
        errorMessages[errorKey]?.en ||
        (language === "zh"
          ? "语音识别错误，请重试"
          : language === "ms"
            ? "Ralat pengecaman suara, sila cuba lagi"
  ...
  return (
    <>
            ...
            <ul className="text-sm text-stone-600 space-y-2 mb-6">
              <li className="flex items-center gap-2">
                ...
                {language === "zh"
                  ? "支持中文、英语和马来语"
                  : language === "ms"
                    ? "Menyokong Mandarin, Inggeris dan Melayu"
                    : "Supports Chinese, English and Malay"}
              </li>
              <li className="flex items-center gap-2">
                <span className="text-emerald-500">✓</span>
                {language === "zh"
                  ? "点击麦克风开始，再次点击停止"
                  : language === "ms"
                    ? "Ketik mikrofon untuk mula, ketik lagi untuk berhenti"
              ...
              <li className="flex items-center gap-2">
                ...
                {language === "zh"
                  ? "需要Chrome或Edge浏览器"
                  : language === "ms"
                    ? "Memerlukan pelayar Chrome atau Edge"
                    : "Requires Chrome or Edge browser"}
              </li>
            </ul>

            <div className="flex gap-3">
              <Button variant="outline" onClick={handleDisclaimerClose} className="flex-1">
      ...
      <Card
        className={`flex flex-col gap-3 md:gap-4 transition-all duration-300 ${
          isMaximized
      ...
      >
        ...
        >
          <form
            ...
            className="flex-1 flex gap-2 w-full md:max-w-4xl md:mx-auto"
          >
            <Button
              type="button"
