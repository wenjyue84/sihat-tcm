...
const analysisSteps = [
    ...
    { icon: MessageSquare, label: 'Evaluating speech patterns...', color: 'from-violet-500 to-purple-500' },
        ...
        <Card className="p-8 bg-gradient-to-br from-white to-green-50/50 border-green-100">
            <div className="flex flex-col items-center space-y-6">
                ...
                <div className="relative">
                    {/* Outer pulsing ring */}
                    <div className="absolute inset-0 rounded-full bg-gradient-to-r from-green-400 to-emerald-500 animate-ping opacity-20"
                        style={{ animationDuration: '2s' }} />

                    {/* Middle rotating ring */}
                    <div className="absolute -inset-2 rounded-full border-2 border-dashed border-green-300 animate-spin"
                        style={{ animationDuration: '8s' }} />

                    {/* Icon container */}
                    <div className={`w-20 h-20 rounded-full bg-gradient-to-br ${analysisSteps[currentStep].color} flex items-center justify-center shadow-xl relative z-10`}>
                ...
                <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
                    {analysisSteps.map((step, idx) => {
                        ...
                        return (
                            <div
                                key={idx}
                                className={`flex items-center gap-2 p-2 rounded-lg transition-all duration-300 ${isActive
                                        ? 'bg-green-50 border border-green-200'
                                        : 'bg-gray-50 border border-gray-100'
                                    }`}
                            >
                                <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${step.color} flex items-center justify-center ${isActive ? 'opacity-100' : 'opacity-50'
                                    }`}>
