...
export function MedicineSelectionModal({
  isOpen,
  onClose,
  onSelect,
  language,
}: MedicineSelectionModalProps) {
  ...
  const filteredItems = useMemo(() => {
    ...
    return Array.from(new Set(allItems)).filter((item) => item.toLowerCase().includes(lowerQuery));
  }, [searchQuery]);
  ...
  const content = {
    ...
    noResults: {
      ...
      zh: "未找到匹配的药物。",
      ms: "Tiada ubat yang sepadan ditemui.",
    },
              ...
              <ScrollArea className="flex-1 p-4 h-[350px]">
                {MEDICINE_CATEGORIES.map((cat) => (
                  <TabsContent
                    key={cat.id}
                    value={cat.id}
                    className="mt-0 grid grid-cols-2 sm:grid-cols-3 gap-3 pb-8"
                  >
