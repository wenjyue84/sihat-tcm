...
import { Camera, X, Flashlight, FlashlightOff, Heart, Activity, AlertCircle, CheckCircle2 } from 'lucide-react'
...
export function CameraPulseSensor({ onBpmDetected, onCancel, t }: CameraPulseSensorProps) {
    ...
    const translations = {
        ...
        flashUnavailable: t.pulse?.cameraMeasurement?.flashUnavailable || 'Flash unavailable - please use manual mode',
        initializing: t.pulse?.cameraMeasurement?.initializing || 'Initializing camera...',
        pressFingerHarder: t.pulse?.cameraMeasurement?.pressFingerHarder || 'Press finger harder on camera',
                ...
                <div className="space-y-4">
                        ...
                        <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                            <motion.div
                                className={`h-full rounded-full ${
                                    signalData.signalQuality >= 80 ? 'bg-emerald-500' :
                                    signalData.signalQuality >= 60 ? 'bg-amber-500' : 'bg-red-400'
                                }`}
                                initial={{ width: 0 }}
                    ...
                    {signalData.filteredSignal.length > 0 && (
                        <div className="h-16 bg-slate-800/50 rounded-lg overflow-hidden relative">
                            ...
                            >
                                <polyline
                                    points={signalData.filteredSignal.map((val, i) => {
