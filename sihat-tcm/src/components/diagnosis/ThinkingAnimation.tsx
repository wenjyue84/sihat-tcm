...
export function ThinkingAnimation({ basicInfo, variant = 'compact' }: ThinkingAnimationProps) {
    ...
    const getBMICategory = (bmi: number) => {
        if (bmi < 18.5) return language === 'zh' ? '偏瘦' : language === 'ms' ? 'Kurang berat' : 'Underweight'
        if (bmi < 25) return language === 'zh' ? '正常' : language === 'ms' ? 'Normal' : 'Normal'
        if (bmi < 30) return language === 'zh' ? '超重' : language === 'ms' ? 'Berlebihan berat' : 'Overweight'
    ...
    const getPatientInfoCards = () => {
        ...
        // Age & Gender
        if (basicInfo.age) {
            const genderText = basicInfo.gender === 'male'
                ? (language === 'zh' ? '男' : language === 'ms' ? 'Lelaki' : 'Male')
                : basicInfo.gender === 'female'
                    ? (language === 'zh' ? '女' : language === 'ms' ? 'Perempuan' : 'Female')
                    : basicInfo.gender
            cards.push({
                icon: Clock,
                label: language === 'zh' ? '年龄与性别' : language === 'ms' ? 'Umur & Jantina' : 'Age & Gender',
        ...
        if (bmi) {
            ...
            cards.push({
                icon: Activity,
                label: language === 'zh' ? 'BMI 体质指数' : language === 'ms' ? 'Indeks Jisim Badan' : 'BMI (Body Mass Index)',
                value: `${bmi} - ${getBMICategory(bmiNum)}`,
                color: bmiNum < 18.5 ? 'text-blue-600' : bmiNum < 25 ? 'text-green-600' : bmiNum < 30 ? 'text-yellow-600' : 'text-red-600',
                bgColor: bmiNum < 18.5 ? 'bg-blue-50' : bmiNum < 25 ? 'bg-green-50' : bmiNum < 30 ? 'bg-yellow-50' : 'bg-red-50',
            })
        }
        ...
        if (basicInfo.symptoms) {
            cards.push({
                icon: AlertCircle,
                label: language === 'zh' ? '主要症状' : language === 'ms' ? 'Simptom Utama' : 'Main Symptoms',
                value: basicInfo.symptoms.length > 50 ? basicInfo.symptoms.substring(0, 47) + '...' : basicInfo.symptoms,
                color: 'text-rose-600',
                bgColor: 'bg-rose-50',
            })
        }
        ...
        if (basicInfo.symptomDuration) {
            cards.push({
                icon: Clock,
                label: language === 'zh' ? '症状持续时间' : language === 'ms' ? 'Tempoh Simptom' : 'Symptom Duration',
    ...
    const allItems = [
        ...patientInfoCards.map((card, idx) => ({ type: 'info' as const, data: card, id: `info-${idx}` })),
        ...thinkingActions.map((action, idx) => ({ type: 'action' as const, data: action, id: `action-${idx}` })),
    ]
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg rounded-bl-none p-4 border border-emerald-200/50">
                <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="text-sm font-medium text-emerald-700">
                        {language === 'zh' ? '医师正在分析...' : language === 'ms' ? 'Doktor sedang menganalisis...' : 'Doctor is analyzing...'}
                    </span>
                </div>

                {/* Current rotating item */}
                <div
                    className={`min-h-[60px] transition-all duration-300 ${fadeIn ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
                        }`}
                >
                    {currentItem?.type === 'info' && currentItem.data && (
                        <div className={`flex items-center gap-3 p-3 rounded-lg ${currentItem.data.bgColor} border border-white/50`}>
                ...
                <div className="flex items-center justify-center gap-1 mt-3">
                    {allItems.slice(0, 8).map((_, i) => (
                        <div
                            key={i}
                            className={`h-1.5 rounded-full transition-all duration-300 ${i === currentIndex % 8 ? 'w-4 bg-emerald-500' : 'w-1.5 bg-emerald-200'
                                }`}
                        />
                    ))}
                </div>
    ...
    return (
        <div className="p-5 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl border border-emerald-200">
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-semibold text-emerald-800">
                            {language === 'zh' ? '医师正在分析您的健康状况...' : language === 'ms' ? 'Doktor sedang menganalisis kesihatan anda...' : 'Doctor is analyzing your health profile...'}
                        </span>
                    </div>
                    <p className="text-sm text-stone-500">
                        {language === 'zh' ? '正在查阅并综合分析您提供的信息' : language === 'ms' ? 'Menyemak dan menganalisis maklumat anda' : 'Reviewing and synthesizing your provided information'}
                    </p>
                </div>
            </div>
            ...
            <div
                className={`min-h-[80px] transition-all duration-300 ${fadeIn ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
                    }`}
            >
                {currentItem?.type === 'info' && currentItem.data && (
                    <div className={`flex items-center gap-4 p-4 rounded-xl ${currentItem.data.bgColor} border border-white shadow-sm`}>
            ...
            <div className="flex items-center justify-center gap-1.5 mt-4">
                {allItems.slice(0, 10).map((_, i) => (
                    <div
                        key={i}
                        className={`h-2 rounded-full transition-all duration-300 ${i === currentIndex % 10 ? 'w-6 bg-emerald-500' : 'w-2 bg-emerald-200'
                            }`}
                    />
                ))}
            </div>
