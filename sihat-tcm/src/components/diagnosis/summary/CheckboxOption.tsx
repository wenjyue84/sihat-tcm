...
export function CheckboxOption({
    ...
    return (
        <div className={`space-y-3 p-2 rounded-lg hover:bg-slate-50 transition-colors ${children ? '' : 'flex items-center space-x-3'}`}>
            <div className="flex items-center space-x-3">
                ...
                />
                <Label htmlFor={id} className="text-sm font-medium text-slate-700 cursor-pointer flex-1 break-words">
                    {label}
                </Label>
            </div>
            {children && checked && (
