    ...
    return (
        <motion.div variants={variants} className="space-y-4">
            <div className="text-center">
                <h3 className="text-emerald-800 font-semibold mb-1">Recommended TCM Practitioners in Malaysia</h3>
                <p className="text-stone-500 text-xs mb-4">Click to view contact details</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {loading ? (
                    <div className="col-span-full text-center py-4 text-stone-500">Loading practitioners...</div>
                ) : doctors.length > 0 ? (
                    doctors.map((doctor) => (
                        <button
                            key={doctor.id}
                        ...
                        >
                            <div className="w-12 h-12 bg-stone-100 rounded-full mb-2 overflow-hidden border border-stone-200 group-hover:border-emerald-200">
                                <img src={doctor.photo || `https://api.dicebear.com/7.x/avataaars/svg?seed=${doctor.name}`} alt={doctor.name} className="w-full h-full object-cover" />
                            </div>
                            <span className="text-xs font-medium text-stone-700 group-hover:text-emerald-700 text-center line-clamp-1">{doctor.name}</span>
                            <span className="text-[10px] text-stone-500 text-center line-clamp-1">{doctor.clinicName}</span>
                        </button>
                    ))
                ) : (
                    <div className="col-span-full text-center py-4 text-stone-500 italic">No practitioners found.</div>
                )}
            </div>
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-emerald-600 font-medium text-sm">{selectedDoctor.clinicName}</p>
                                    <p className="text-stone-500 text-xs">{selectedDoctor.experience} Experience</p>
                                </div>
                            </div>

                            <div className="space-y-3">
                                <div>
                                    <h5 className="text-xs font-semibold text-stone-500 uppercase mb-1">Specialties</h5>
                                    <div className="flex flex-wrap gap-1">
                                        {selectedDoctor.specialties.map((spec, idx) => (
                                            <span key={idx} className="bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded text-xs border border-emerald-100">
                                                {spec}
                                            </span>
                                        ))}
                                    </div>
