...
export const downloadPDF = async (
    ...
    if (patientInfo) {
        ...
        if (reportOptions.includePatientName !== false && patientInfo.name) addWrappedText(`Name: ${patientInfo.name}`, 11)
        if (reportOptions.includePatientAge !== false && patientInfo.age) addWrappedText(`Age: ${patientInfo.age} years`, 11)
        if (reportOptions.includePatientGender !== false && patientInfo.gender) addWrappedText(`Gender: ${patientInfo.gender}`, 11)
        if (reportOptions.includePatientContact && patientInfo.contact) addWrappedText(`Contact: ${patientInfo.contact}`, 11)
        if (reportOptions.includePatientAddress && patientInfo.address) addWrappedText(`Address: ${patientInfo.address}`, 11)
        if (reportOptions.includeEmergencyContact && patientInfo.emergencyContact) addWrappedText(`Emergency Contact: ${patientInfo.emergencyContact}`, 11)
