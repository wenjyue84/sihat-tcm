...
export function DietaryRecommendations({ foodRecs, avoidRecs, recipes, includeDietary, onSectionClick }: DietaryRecommendationsProps) {
    const item = {
        hidden: { opacity: 0, y: 20 },
        show: { opacity: 1, y: 0 }
    }

    if (!includeDietary || (foodRecs.length === 0 && avoidRecs.length === 0 && recipes.length === 0)) {
        return null;
    }
            ...
            <Card className="h-full">
                <CardHeader className="pb-2">
                    <CardTitle className="flex items-center gap-2 text-stone-800">
                        <button
                            onClick={() => onSectionClick("Please elaborate on the Dietary Therapy recommendations. Why are these foods beneficial for me?")}
