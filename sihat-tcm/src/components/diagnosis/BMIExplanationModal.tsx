...
export function BMIExplanationModal({
  isOpen,
  onClose,
  bmi,
  height,
  weight,
}: BMIExplanationModalProps) {
  const { t } = useLanguage();
  ...
  const getBMICategory = (value: number) => {
    if (value < 18.5)
      return {
        label: t.basicInfo.bmiExplanation.underweight,
        color: "text-blue-500",
        bg: "bg-blue-500",
        range: "< 18.5",
      };
    if (value < 25)
      return {
        label: t.basicInfo.bmiExplanation.normal,
    ...
    if (value < 30)
      return {
        label: t.basicInfo.bmiExplanation.overweight,
        color: "text-orange-500",
        bg: "bg-orange-500",
        range: "25 - 29.9",
      };
    return {
      label: t.basicInfo.bmiExplanation.obese,
      color: "text-red-500",
      bg: "bg-red-500",
      range: "≥ 30",
    };
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="flex flex-col items-center justify-center p-6 bg-stone-50 rounded-2xl border border-stone-100">
            ...
            <div className="flex items-baseline gap-2">
              <span className={`text-5xl font-bold ${category.color}`}>{bmi.toFixed(1)}</span>
            </div>
            <span
              className={`mt-2 px-3 py-1 rounded-full text-sm font-semibold ${category.color} bg-white shadow-sm border border-stone-100`}
            >
              {category.label}
            </span>
          </div>
          ...
          <div className="space-y-2">
            <div className="h-4 bg-stone-100 rounded-full overflow-hidden relative flex">
              ...
              {/* Normal Segment */}
              <div
                className="h-full bg-emerald-300 flex-[1.5] border-r border-white/50"
                title="18.5 - 24.9"
              />
              {/* Overweight Segment */}
              <div
                className="h-full bg-orange-300 flex-[1] border-r border-white/50"
                title="25 - 29.9"
              />
              {/* Obese Segment */}
