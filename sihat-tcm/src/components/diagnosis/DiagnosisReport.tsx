...
export function DiagnosisReport({ data, patientInfo, reportOptions, smartConnectData, onRestart, saved }: DiagnosisReportProps) {
    ...
    const downloadPDF = async (languageKey: Language = 'en') => {
        ...
        const addWrappedText = (text: string, fontSize: number, isBold = false) => {
            ...
            try { doc.setFont(fontName, 'normal') } catch (e) { doc.setFont('helvetica', isBold ? 'bold' : 'normal') }
    ...
    const shareReport = async () => {
        ...
        if (navigator.share) {
            try { await navigator.share({ title: 'TCM Report', text: shareText, url: 'https://sihat-tcm.vercel.app' }) }
            catch (e) {
                console.error(e)
    ...
    const handleSaveToHistory = async () => {
        if (!user) {
            localStorage.setItem('pendingDiagnosisSave', JSON.stringify({ data, patientInfo, reportOptions, smartConnectData }))
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-4 border-b border-emerald-100 overflow-y-auto max-h-[35vh]">
            <div className="max-w-lg mx-auto space-y-3">
                <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setIsChatExpanded(false)} className="text-xs px-3 py-1.5 bg-white/80 text-emerald-700 rounded-full border border-emerald-200 hover:bg-white transition-colors">
    ...
    const content = (
        <div className={`transition-all duration-300 ${isChatExpanded ? 'fixed inset-0 z-50 bg-stone-50 flex flex-col md:flex-row overflow-hidden' : ''}`}>
            {isChatExpanded && <div className="md:hidden"><MobileReportSummary /></div>}
                ...
                <motion.div variants={item}>
                    <div className="bg-stone-100 rounded-xl p-4 text-center">
                        <p className="text-stone-600 text-xs md:text-sm">
                            {data.disclaimer || "This report is generated by Sihat TCM AI Assistant for informational purposes only. It is not a substitute for professional medical advice. Please consult a licensed TCM practitioner for diagnosis and treatment."}
                        </p>
                    </div>
                </motion.div>
            ...
            {!isChatExpanded && (
                <>
                    ...
                    <ReportChatWindow
                        ...
                        onMessageSent={() => setActiveQuestion(null)}
                        isExpanded={false}
                        onToggleExpand={() => { setIsChatOpen(false); setIsChatExpanded(true); }}
                        messages={chatMessages}
                        onMessagesChange={setChatMessages}
                    />
                </>
