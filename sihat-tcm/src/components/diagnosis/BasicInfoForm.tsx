...
export function BasicInfoForm({ onComplete, initialData }: { onComplete: (data: BasicInfoData) => void, initialData?: BasicInfoData }) {
    ...
    const validateStep = (step: number, scrollToField: boolean = false): boolean => {
        ...
        switch (step) {
            case 1:
                ...
                if (!formData.gender) {
                    setStepError(t.basicInfo.gender + ' ' + (t.errors?.requiredField || 'This field is required'))
                    return false
                }
                if (!formData.age) {
                    setStepError(t.basicInfo.age + ' ' + (t.errors?.requiredField || 'This field is required'))
            ...
            case 2:
                ...
                if (!formData.symptomDuration) {
                    setStepError(t.basicInfo.duration + ' ' + (t.errors?.requiredField || 'This field is required'))
    ...
    return (
        <Card className="overflow-hidden border-none shadow-2xl bg-white/90 backdrop-blur-sm ring-1 ring-stone-900/5">
            ...
            <form onSubmit={handleSubmit} className="p-6 pb-24 md:pb-6" noValidate suppressHydrationWarning>
                {/* Progress Indicator */}
