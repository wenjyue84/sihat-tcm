...
export function useDeviceInfo(): DeviceInfo {
  const [deviceInfo, setDeviceInfo] = useState<DeviceInfo>(() => 
    platformOptimizer.getDeviceInfo()
  )
...
export function useBreakpoint(breakpoint: keyof ResponsiveBreakpoints): boolean {
  const [matches, setMatches] = useState(() => 
    platformOptimizer.isBreakpoint(breakpoint)
  )
...
export function usePlatformClasses(): string[] {
  ...
  return useMemo(() => 
    ...
    [deviceInfo.type, deviceInfo.platform, deviceInfo.browser, deviceInfo.hasTouch, deviceInfo.isStandalone]
  )
}
