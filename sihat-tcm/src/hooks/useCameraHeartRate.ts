 ...
 */
function findDominantFrequency(signal: number[], sampleRate: number): { frequency: number; magnitude: number } {
    const n = signal.length
 ...
 */
function calculateBPM(signalBuffer: number[], sampleRate: number): { bpm: number | null; quality: number } {
    ...
    const filtered = bandpassFilter(detrended, sampleRate, CONFIG.MIN_FREQUENCY, CONFIG.MAX_FREQUENCY)
    ...
    const quality = Math.min(100, Math.round(
        (magnitude / (variance * signalBuffer.length)) * 100 * 2
    ))
...
// Main Hook
// ============================================================================
export function useCameraHeartRate(options: UseCameraHeartRateOptions = {}): UseCameraHeartRateReturn {
    ...
    const processFrame = useCallback(() => {
        ...
        if (frameCountRef.current % 10 === 0 && signalBufferRef.current.length >= CONFIG.STABILIZATION_WINDOW) {
 ...
 */
export function testBPMCalculation(targetBpm: number): { calculatedBpm: number | null; quality: number; error: number } {
