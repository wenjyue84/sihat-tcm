...
const FALLBACK_MODELS = ["gemini-2.5-flash", "gemini-2.0-flash-exp", "gemini-2.0-flash"];
...
export async function POST(req: Request) {
  try {
    ...
    devLog(
      "info",
      "API/western-chat",
      `Received request with model: ${model}, language: ${language}`
    );
    ...
    try {
      const result = streamText({
        ...
        onFinish: (completion) => {
          devLog(
            "info",
            "API/western-chat",
            `Stream finished with ${model}. Text length: ${completion.text.length}`
          );
        },
      });
      ...
      for (const fallbackModel of FALLBACK_MODELS) {
        ...
        try {
          ...
          const fallbackResult = streamText({
            ...
            onFinish: (completion) => {
              devLog(
                "info",
                "API/western-chat",
                `Fallback ${fallbackModel} finished. Text length: ${completion.text.length}`
              );
            },
          });
          ...
          devLog("error", "API/western-chat", `Fallback ${fallbackModel} also failed`, {
...
function buildTCMContext(
  reportData: Record<string, unknown> | undefined,
  patientInfo: Record<string, unknown> | undefined
): string {
  let context = "";
  ...
  if (reportData.diagnosis) {
    const diagnosis =
      typeof reportData.diagnosis === "string"
        ...
        : (reportData.diagnosis as Record<string, unknown>)?.primary_pattern ||
          JSON.stringify(reportData.diagnosis);
    ...
    if (typeof reportData.diagnosis === "object" && reportData.diagnosis !== null) {
      ...
      if (
        Array.isArray(diagnosisObj.secondary_patterns) &&
        diagnosisObj.secondary_patterns.length > 0
      ) {
  ...
  if (reportData.constitution) {
    const constitution =
      typeof reportData.constitution === "string"
        ...
        : (reportData.constitution as Record<string, unknown>)?.type ||
          JSON.stringify(reportData.constitution);
  ...
  if (reportData.analysis) {
    const analysis =
      typeof reportData.analysis === "string"
        ? reportData.analysis
        : (reportData.analysis as Record<string, unknown>)?.summary ||
