...
function checkBlogImages(): SystemAlert[] {
    ...
    const files = fs.readdirSync(postsDirectory).filter(f => f.endsWith('.mdx') && !f.includes('.ms.') && !f.includes('.zh.'));
...
function checkEnvironmentVariables(): SystemAlert[] {
    ...
    const criticalVars = [
        { name: 'GOOGLE_GENERATIVE_AI_API_KEY', alias: 'Gemini API Key', solution: 'Get an API key from https://aistudio.google.com/app/apikey and add it to your .env.local file.' },
        { name: 'NEXT_PUBLIC_SUPABASE_URL', alias: 'Supabase URL', solution: 'Get your Supabase project URL from https://supabase.com/dashboard and add it to .env.local.' },
        { name: 'NEXT_PUBLIC_SUPABASE_ANON_KEY', alias: 'Supabase Anon Key', solution: 'Get your Supabase anon key from the project settings and add it to .env.local.' },
    ];
    ...
    try {
        ...
        if (promptsError) {
            ...
            alerts.push({
                ...
                severity: 'error',
                message,
                solution: 'Check your Supabase connection and ensure the system_prompts table exists with the correct schema.',
            });
            return alerts;
        }
