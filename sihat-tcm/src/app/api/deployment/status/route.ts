...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { environment, version, commit, status, type = "deployment", metadata = {} } = body;
...
async function getHealthStatus(): Promise<{
  ...
  try {
    ...
    const healthResponse = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3100"}/api/monitoring/health`,
      {
        method: "GET",
        headers: {
...
async function getPerformanceMetrics(): Promise<{
  ...
  try {
    ...
    const availability =
      totalRequests > 0 ? ((totalRequests - errorCount) / totalRequests) * 100 : 100;
 ...
 * Get deployment history
 */
async function getDeploymentHistory(environment: string): Promise<
  Array<{
    timestamp: string;
    version: string;
    commit: string;
