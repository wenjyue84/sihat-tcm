...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: pendingNotifications } = await supabase.rpc("get_pending_notifications", {
      p_user_id: user.id,
    });
    ...
    if (syncData) {
      ...
      if (syncData.notificationHistory && Array.isArray(syncData.notificationHistory)) {
        ...
        if (historyToInsert.length > 0) {
          await supabase.from("notification_history").insert(historyToInsert);
        }
      }
      ...
      if (syncData.scheduledNotifications && Array.isArray(syncData.scheduledNotifications)) {
        ...
        if (notificationsToInsert.length > 0) {
          await supabase.from("scheduled_notifications").insert(notificationsToInsert);
        }
      }
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [preferencesResult, devicesResult, pendingResult, scheduledResult, historyResult] =
      await Promise.all([
        supabase.from("user_notification_preferences").select("*").eq("user_id", user.id).single(),

        supabase.from("user_devices").select("*").eq("user_id", user.id).eq("is_active", true),
