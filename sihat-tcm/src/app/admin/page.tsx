...
export default function AdminDashboard() {
    ...
    const [expandedPrompts, setExpandedPrompts] = useState<Record<PromptType, boolean>>({
        chat: false, tongue: false, face: false, body: false, listening: false, inquiry_summary: false, final: false,
    });
    ...
    useEffect(() => { fetchAllPrompts(); fetchAdminSettings(); }, []);
        ...
        try {
            ...
            if (data) {
                ...
                data.forEach((item) => {
                    ...
                    if (item.role === 'doctor_chat') newPrompts.chat = text || PROMPT_TYPES.chat.defaultPrompt;
                    else if (item.role === 'doctor_tongue') newPrompts.tongue = text || PROMPT_TYPES.tongue.defaultPrompt;
                    else if (item.role === 'doctor_face') newPrompts.face = text || PROMPT_TYPES.face.defaultPrompt;
                    else if (item.role === 'doctor_body') newPrompts.body = text || PROMPT_TYPES.body.defaultPrompt;
                    else if (item.role === 'doctor_listening') newPrompts.listening = text || PROMPT_TYPES.listening.defaultPrompt;
                    else if (item.role === 'doctor_inquiry_summary') newPrompts.inquiry_summary = text || PROMPT_TYPES.inquiry_summary.defaultPrompt;
                    else if (item.role === 'doctor_final') newPrompts.final = text || PROMPT_TYPES.final.defaultPrompt;
                    else if (item.role === 'doctor' && item.config) setDoctorLevel(item.config.default_level || 'Physician');
                });
    ...
    const handleSavePrompt = async (type: PromptType) => {
        ...
        try {
            const { data: existing } = await supabase.from('system_prompts').select('id').eq('role', promptConfig.role).single();
            if (existing) {
                await supabase.from('system_prompts').update({ prompt_text: prompts[type], updated_at: new Date() }).eq('role', promptConfig.role);
            } else {
                await supabase.from('system_prompts').insert([{ role: promptConfig.role, prompt_text: prompts[type] }]);
            }
            setSaved(type);
    ...
    const handleSaveMusicConfig = async () => {
        ...
        try {
            ...
            await fetch('/api/admin/settings', {
                method: 'PUT',
                headers,
                body: JSON.stringify({ backgroundMusicEnabled: musicEnabled, backgroundMusicUrl: musicUrl, backgroundMusicVolume: musicVolume })
            });
    ...
    const handleSaveConfig = async () => {
        setSaving('config');
        try {
            const { data: existing } = await supabase.from('system_prompts').select('id').eq('role', 'doctor').single();
            const config = { default_level: doctorLevel, model: DOCTOR_MODEL_MAPPING[doctorLevel].model };
            if (existing) {
                await supabase.from('system_prompts').update({ config, updated_at: new Date() }).eq('role', 'doctor');
            } else {
            ...
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
                <Card className="max-w-md w-full border-red-100 shadow-xl">
                    ...
                    <CardContent className="text-center space-y-4 pt-4">
                        <p className="text-sm text-slate-600">You are currently logged in as <span className="font-bold text-slate-900 capitalize">{profile?.role || 'User'}</span>.</p>
                        <Button onClick={() => router.push('/')} className="w-full bg-slate-900 hover:bg-slate-800">Return Home</Button>
                    </CardContent>
                </Card>
            </div>
        );
            ...
            >
                <div className="h-16 flex items-center justify-between px-6 border-b border-slate-100">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-lg tracking-tight">Admin <span className="text-slate-500">Portal</span></span>
                    </div>
                ...
                <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
                    <div className="space-y-1">
                        <p className="px-4 py-2 text-xs font-bold text-slate-400 uppercase tracking-wider">System</p>
                        <button
                    ...
                    <div className="mt-6 space-y-1">
                        <p className="px-4 py-2 text-xs font-bold text-slate-400 uppercase tracking-wider">Management</p>
                        <button
                        ...
                        <button
                            onClick={() => { setActiveTab('practitioners'); setIsMobileMenuOpen(false); }}
                ...
                <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mt-0.5 hidden sm:block">Sihat Medical Administration • v2.4.0</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <Button
                    ...
                    <div className="max-w-5xl mx-auto pb-20">
                        {activeTab === 'prompts' && (
                            <div className="space-y-6 animate-in fade-in-50 slide-in-from-bottom-2 duration-300">
                                <Card className="border-none shadow-md bg-white">
                                    ...
                                    <CardContent className="pt-6">
                                        <div className="flex flex-wrap items-end gap-4 p-1">
                                                    ...
                                                    <SelectContent>
                                                        <SelectItem value="Physician">
                                                            <div className="flex flex-col">
                                                                ...
                                                                <span className="text-xs text-slate-500">gemini-2.0-flash • Fast response, general care</span>
                                                            </div>
                                                        </SelectItem>
                                                        <SelectItem value="Expert">
                                                            <div className="flex flex-col">
                                                                <span className="font-medium">🩺 Expert (Advanced)</span>
                                                                <span className="text-xs text-slate-500">gemini-2.5-pro • Deeper reasoning, complex cases</span>
                                                            </div>
                                                        </SelectItem>
                                                        <SelectItem value="Master">
                                                            <div className="flex flex-col">
                                                                <span className="font-medium">👨‍⚕️ Master (Premium)</span>
                                                                <span className="text-xs text-slate-500">gemini-3.0-preview • Highest accuracy, research grade</span>
                                                            </div>
                                                        </SelectItem>
                                                    </SelectContent>
                                                </Select>
                                            ...
                                            >
                                                {saving === 'config' ? <Loader2 className="w-4 h-4 animate-spin" /> : saved === 'config' ? <Check className="w-4 h-4" /> : 'Save Configuration'}
                                            </Button>
                                        </div>
                                    </CardContent>
                                </Card>
                                ...
                                <div className="space-y-6 pt-4">
                                    ...
                                    {[
                                        { id: '1', title: 'Step 1: Patient Inquiry', prompts: ['chat', 'inquiry_summary'] as PromptType[] },
                                        { id: '2', title: 'Step 2: AI Visual & Audio Analysis', prompts: ['tongue', 'face', 'body', 'listening'] as PromptType[] },
                                        { id: '3', title: 'Step 3: Diagnosis Generation', prompts: ['final'] as PromptType[] },
                                    ].map((group) => (
                                        <div key={group.id} className="space-y-4">
                                            <p className="text-xs font-bold uppercase tracking-widest text-slate-400 pl-1">{group.title}</p>
                                            <div className="grid gap-4">
                                                {group.prompts.map(type => {
                                                    ...
                                                    return (
                                                        <Card key={type} className={`border border-slate-200 transition-all duration-300 ${isExpanded ? 'ring-2 ring-slate-900 shadow-lg' : 'hover:border-slate-300 hover:shadow-sm'}`}>
                                                            <div
                                                                className="p-5 flex items-center justify-between cursor-pointer"
                                                                onClick={() => togglePrompt(type)}
                                                            >
                                                                <div className="flex items-center gap-4">
                                                                    <div className={`p-2.5 rounded-lg ${isExpanded ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-500'}`}>
                                                                ...
                                                                <div className="px-5 pb-5 pt-0 animate-in slide-in-from-top-2">
                                                                    <div className="rounded-lg border border-slate-200 bg-slate-50 overflow-hidden">
                                                                        <div className="flex items-center justify-between px-4 py-2 bg-slate-100 border-b border-slate-200">
                                                                            ...
                                                                            {prompts[type] !== config.defaultPrompt && <span className="text-[10px] font-bold bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">CUSTOMIZED</span>}
                                                                        </div>
                                                                        <Textarea
                                                                            value={prompts[type]}
                                                                    ...
                                                                    <div className="flex justify-between items-center mt-4">
                                                                        <Button variant="ghost" size="sm" onClick={() => handleResetToDefault(type)} className="text-slate-500 hover:text-red-600">Revert to Default</Button>
                                                                        <Button onClick={() => handleSavePrompt(type)} disabled={saving === type} className="bg-slate-900 hover:bg-slate-800">
                        ...
                        {activeTab === 'config' && (
                            <div className="space-y-6 animate-in fade-in-50 slide-in-from-bottom-2 duration-300">
                                <Card>
                                    <CardHeader>
                                        <CardTitle className="flex items-center gap-2"><Music className="w-5 h-5" /> Environment Ambience</CardTitle>
                                        <CardDescription>Configure background audio for the waiting room experience.</CardDescription>
                                    </CardHeader>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <Button variant="outline" size="icon" onClick={handleTestMusic}>
                                                    {isTestPlaying ? <Loader2 className="w-4 h-4 animate-spin text-slate-900" /> : <Mic className="w-4 h-4" />}
                                                </Button>
                                            </div>
                                        </div>
                                        <div className="space-y-3">
                                            ...
                                            <Slider value={[musicVolume]} max={1} step={0.01} onValueChange={(val) => setMusicVolume(val[0])} />
                                        </div>
                                    </CardContent>
                                    <CardFooter className="bg-slate-50 border-t border-slate-100 flex justify-end p-4">
                                        <Button onClick={handleSaveMusicConfig} disabled={saving === 'config_music'} className="bg-slate-900">
                        ...
                        {activeTab === 'security' && <div className="animate-in fade-in-50 slide-in-from-bottom-2"><SecuritySettings /></div>}
                        {activeTab === 'users' && <div className="animate-in fade-in-50 slide-in-from-bottom-2"><UserManager /></div>}
                        {activeTab === 'practitioners' && <div className="animate-in fade-in-50 slide-in-from-bottom-2"><PractitionerManager /></div>}
                            ...
                            <div className="animate-in fade-in-50 slide-in-from-bottom-2">
                                <Card className="border-none shadow-lg bg-gradient-to-br from-indigo-50 to-white overflow-hidden relative">
                                    ...
                                    <CardHeader className="text-center pb-8 pt-10 relative z-10">
                                        ...
                                        <CardTitle className="text-3xl font-bold text-slate-900">Content Management System</CardTitle>
                                        ...
                                        <div className="bg-white p-6 rounded-xl border border-emerald-100 shadow-sm flex flex-col items-center text-center space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-slate-500 max-w-sm mt-1">
                                                    Select the <code>.apk</code> file from your computer. It will be automatically renamed to <code>sihat-tcm.apk</code> and deployed to the public server.
                                                </p>
                                            </div>
            ...
            <div className="p-4 flex gap-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-amber-800 mt-1">
                        Any changes made to System Prompts will take effect immediately for all new consult sessions.
                        Please test changes rigorously in the <span className="underline cursor-pointer font-medium" onClick={() => window.open('/test-runner', '_blank')}>Test Runner</span> before deploying to production.
                    </p>
                </div>
            </div>
        </Card>
