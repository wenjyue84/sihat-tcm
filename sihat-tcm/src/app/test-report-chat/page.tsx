...
const MOCK_REPORT_DATA = {
    ...
    analysis: {
        summary: "Patient presents with classic signs of Spleen Qi deficiency with dampness accumulation",
        pattern_rationale: "Fatigue, poor appetite, loose stools, and tongue with teeth marks indicate weak Spleen Qi"
    },
    recommendations: {
        food_therapy: {
...
export default function TestReportChatPage() {
    ...
    const testAllModels = async () => {
        ...
        for (const model of TEST_MODELS) {
            ...
            try {
                ...
                setLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] ✅ ${model.id}: SUCCESS (${Date.now() - start}ms) - Got ${responseText.length} chars`]);

            } catch (err: any) {
                results.push({ model: model.id, success: false, time: Date.now() - start, error: err.message });
                setLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] ❌ ${model.id}: FAILED - ${err.message}`]);
            }
        }

        // Summary
        const successCount = results.filter(r => r.success).length;
        setLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] ==== SUMMARY: ${successCount}/${results.length} models working ====`]);
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto min-h-screen bg-gray-900 text-white">
            ...
            <p className="text-gray-400 mb-6">Test the &quot;Ask About Your Report&quot; AI feature with different models</p>
            ...
            {/* Mock Data Preview */}
            <div className="bg-gray-800 rounded-xl p-6 mb-6 border border-gray-700">
                <h2 className="text-xl font-semibold mb-4 text-emerald-300">📋 Mock Report Data Being Used</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="bg-gray-700/50 p-4 rounded-lg">
                        <h3 className="font-semibold text-emerald-200 mb-2">Patient Info</h3>
                        <p><span className="text-gray-400">Name:</span> {MOCK_PATIENT_INFO.name}</p>
                        <p><span className="text-gray-400">Age:</span> {MOCK_PATIENT_INFO.age}</p>
                        <p><span className="text-gray-400">Gender:</span> {MOCK_PATIENT_INFO.gender}</p>
                        <p><span className="text-gray-400">Chief Complaint:</span> {MOCK_PATIENT_INFO.symptoms}</p>
                    </div>
                    <div className="bg-gray-700/50 p-4 rounded-lg">
                        <h3 className="font-semibold text-emerald-200 mb-2">Diagnosis</h3>
                        <p><span className="text-gray-400">Primary Pattern:</span> {MOCK_REPORT_DATA.diagnosis.primary_pattern}</p>
                        <p><span className="text-gray-400">Constitution:</span> {MOCK_REPORT_DATA.constitution.type}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
                ...
                <div className="space-y-2 text-sm text-gray-300">
                    <p><strong className="text-emerald-200">gemini-2.0-flash</strong> - Latest, fast, agentic capabilities (Dec 2024)</p>
                    <p><strong className="text-emerald-200">gemini-2.5-flash</strong> - Enhanced flash model with better reasoning</p>
                    <p><strong className="text-emerald-200">gemini-2.5-pro</strong> - Most advanced, best for complex reasoning</p>
                    <p className="text-yellow-400 mt-4">💡 The chatbot should use the model selected by the patient via DoctorLevel</p>
                </div>
            </div>
        </div>
    );
