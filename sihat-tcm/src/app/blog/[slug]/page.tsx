...
export default async function BlogPost(props: Props) {
    ...
    const allPosts = getPostSlugs().map((slug) => getPostBySlug(slug, ['title', 'slug', 'coverImage', 'date'], lang));
    const relatedPosts = allPosts
        .filter((p) => p.slug !== post.slug)
        .slice(0, 3);

    // Extract headings for Table of Contents
    const headings: Heading[] = post.content.match(/^##\s+(.+)$/gm)?.map((heading: string) => {
        const title = heading.replace(/^##\s+/, '');
        // Use Unicode property escapes to support generic characters (Chinese, Malay, etc.)
        const id = title.toLowerCase().trim().replace(/[^\p{L}\p{N}\-_]+/gu, '-').replace(/^-+|-+$/g, '');
    ...
    const components = {
        ...
        h2: (props: any) => {
            ...
            const id = text.toLowerCase().trim().replace(/[^\p{L}\p{N}\-_]+/gu, '-').replace(/^-+|-+$/g, '') || 'heading';
            return <h2 id={id} {...props} className="text-2xl font-serif font-semibold text-stone-800 mt-8 mb-4 border-b border-stone-200 pb-2 scroll-mt-24" />
        },
        h3: (props: any) => (
    ...
    const jsonLd = {
        ...
        headline: post.title,
        description: post.excerpt,
        image: post.coverImage ? `https://sihat-tcm.vercel.app${post.coverImage}` : 'https://sihat-tcm.vercel.app/og-image.png',
    ...
    const blogTranslations = {
        en: {
            ...
            promoDesc: 'Sihat TCM uses advanced AI to analyze your tongue and provide immediate health insights just like a TCM practitioner.',
        ...
        ms: {
            ...
            promoDesc: 'Sihat TCM menggunakan AI canggih untuk menganalisis lidah anda dan memberikan wawasan kesihatan segera seperti pengamal TCM.',
        ...
        <article className="min-h-screen bg-stone-50/30 pb-20">
            ...
            <div className="bg-white border-b border-stone-100 mb-12">
                <div className="container px-4 mx-auto py-12 max-w-4xl">
                    ...
                    <nav className="flex items-center text-sm text-stone-500 mb-8" aria-label="Breadcrumb">
                        ...
                        <Link href="/blog" className="hover:text-emerald-700 transition-colors">{t.blog}</Link>
                        <span className="mx-2 text-stone-300">/</span>
                        <span className="text-stone-900 line-clamp-1 max-w-[200px] md:max-w-none font-medium truncate" aria-current="page">
                            {post.title}
                        </span>
                    </nav>

                    <Button asChild variant="ghost" className="mb-4 pl-0 text-stone-500 hover:text-stone-900 hover:bg-transparent -ml-2">
                        <Link href="/blog">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-start">
                            <div className="flex flex-wrap gap-2">
                                <Badge variant="secondary" className="bg-emerald-100 text-emerald-800 hover:bg-emerald-200 border-none">{t.analysisBadge}</Badge>
                                <Badge variant="secondary" className="bg-stone-100 text-stone-600 border-none">{t.tcmBadge}</Badge>
                            </div>
            ...
            <div className="container px-4 mx-auto max-w-6xl flex flex-col lg:flex-row gap-12">
                ...
                <aside className="hidden lg:block w-64 shrink-0 transition-opacity">
                    <div className="sticky top-24 space-y-8">
                        ...
                        <div className="border border-emerald-100 bg-emerald-50 rounded-xl p-6">
                            ...
                            <Button asChild size="sm" className="w-full bg-emerald-700 hover:bg-emerald-800 text-white rounded-full">
                ...
                <div className="flex-1 max-w-3xl min-w-0">
                    ...
                    <SocialShare url={`https://sihat-tcm.vercel.app/blog/${params.slug}`} title={post.title} />
                    ...
                    <div className="pt-10 border-t border-stone-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {relatedPosts.map((related: any) => (
                                <Link key={related.slug} href={`/blog/${related.slug}${lang !== 'en' ? `?lang=${lang}` : ''}`} className="group block">
                    ...
                    <div className="mt-16 pt-8 border-t border-stone-200">
                        <div className="bg-emerald-50 rounded-2xl p-8 text-center">
                            <h3 className="font-serif text-2xl font-bold text-emerald-900 mb-2">{t.promoTitle}</h3>
            ...
            <FloatingShareButton url={`https://sihat-tcm.vercel.app/blog/${params.slug}`} title={post.title} />
        </article>
    );
}
