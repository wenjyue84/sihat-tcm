...
export default function ViewMedicalReportPage() {
    ...
    const renderMarkdown = (md: string) => {
        ...
        // Headers
        html = html.replace(/^### (.*$)/gim, '<h3 class="text-lg font-semibold text-stone-800 mt-6 mb-3">$1</h3>')
        html = html.replace(/^## (.*$)/gim, '<h2 class="text-xl font-bold text-stone-900 mt-8 mb-4 pb-2 border-b border-stone-200">$1</h2>')
        html = html.replace(/^# (.*$)/gim, '<h1 class="text-3xl font-bold text-stone-900 mb-6">$1</h1>')
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.trim().startsWith('|') && line.trim().endsWith('|')) {
                if (!inTable) {
                    processedLines.push('<div class="overflow-x-auto my-6"><table class="min-w-full divide-y divide-stone-200 border border-stone-200">')
                    inTable = true
                }
                ...
                if (isHeader && !processedLines.some(l => l.includes('<thead'))) {
                    ...
                    cells.forEach(cell => {
                        processedLines.push(`<th class="px-4 py-3 text-left text-xs font-semibold text-stone-700 uppercase tracking-wider border-r border-stone-200 last:border-r-0">${cell.trim()}</th>`)
                    })
                    processedLines.push('</tr>')
                    ...
                    cells.forEach(cell => {
                        processedLines.push(`<td class="px-4 py-3 text-sm text-stone-700 border-r border-stone-200 last:border-r-0">${cell.trim()}</td>`)
                    })
                    processedLines.push('</tr>')
                }
            } else {
        ...
        html = html.replace(/^(?!<[^>]+>)(.+)$/gim, '<p class="text-stone-700 leading-relaxed my-2">$1</p>')
