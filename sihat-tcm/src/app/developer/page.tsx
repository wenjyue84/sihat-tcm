    ...
    const [apiGroups, setApiGroups] = useState([
        {
            ...
            endpoints: [
                { path: '/api/analyze-image', method: 'POST', type: 'AI Analysis', status: 200, latency: '1.2s' },
                { path: '/api/analyze-audio', method: 'POST', type: 'AI Analysis', status: 200, latency: '0.8s' },
            ]
        },
        {
            ...
            endpoints: [
                ...
                { path: '/api/consult', method: 'POST', type: 'Core Consultation', status: 200, latency: '650ms' },
                { path: '/api/ask-dietary-advice', method: 'POST', type: 'AI Advice', status: 200, latency: '1.5s' },
            ]
        },
        {
            ...
            endpoints: [
                { path: '/api/summarize-reports', method: 'POST', type: 'Processing', status: 200, latency: '1.1s' },
                { path: '/api/validate-medicine', method: 'POST', type: 'Safety Check', status: 200, latency: '300ms' },
                { path: '/api/generate-infographic', method: 'POST', type: 'Content Gen', status: 200, latency: '2.4s' },
            ]
        },
        {
            title: 'Admin & System',
            ...
            endpoints: [
                { path: '/api/admin/db-status', method: 'GET', type: 'System Monitor', status: 200, latency: '120ms' },
                { path: '/api/admin/settings', method: 'GET', type: 'Configuration', status: 200, latency: '85ms' },
            ]
        }
    ])
        ...
        try {
            ...
            switch (key) {
                case 'NEXT_PUBLIC_SUPABASE_URL':
                    ...
                    message = success ? `Connected (${dbData.tables?.length || 0} tables)` : 'Connection failed'
                    break
            ...
            setEnvTestMessage(prev => ({ ...prev, [key]: error instanceof Error ? error.message : 'Test failed' }))
        }
    }
    ...
    if (authLoading) return (
        <div className="min-h-screen bg-[#0a0a0b] flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-violet-200/50 font-mono text-sm animate-pulse">Initializing Debugger...</p>
            </div>
        </div>
    )
        ...
        <div className="flex flex-col h-screen bg-[#0a0a0b] text-gray-300 font-sans selection:bg-violet-500/30 overflow-hidden">
            ...
            <header className="h-16 border-b border-white/5 bg-[#0f0f11]/80 backdrop-blur-md flex items-center justify-between px-6 shrink-0 z-50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-base font-bold text-white tracking-tight leading-none">Developer <span className="text-violet-400">Portal</span></h1>
                        <div className="flex items-center gap-2 mt-1">
                            <span className="flex h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
                            <span className="text-[9px] uppercase tracking-widest text-emerald-500/80 font-bold">System Online</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden md:flex flex-col items-end">
                        ...
                        <span className="text-[10px] text-gray-600 uppercase tracking-wider">{currentTime.toLocaleDateString()}</span>
                    </div>
            ...
            <Tabs defaultValue="overview" className="flex-1 flex overflow-hidden">
                {/* Sidebar Navigation */}
                <aside
                    className={`w-64 bg-[#0f0f11] border-r border-white/5 flex flex-col p-4 gap-2 shrink-0 overflow-y-auto fixed inset-y-0 left-0 z-50 transition-transform duration-300 ease-in-out md:relative md:translate-x-0 h-full ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
                        }`}
                >
                    ...
                    <div className="text-xs font-bold text-gray-500 uppercase tracking-widest px-4 py-2 mb-2">Platform</div>
                    ...
                    <div className="text-xs font-bold text-gray-500 uppercase tracking-widest px-4 py-2 mt-6 mb-2">Deployment</div>
                    <TabsList className="bg-transparent flex flex-col h-auto p-0 gap-1 w-full justify-start">
                        <TabsTrigger
                            value="settings"
                            onClick={() => setIsMobileMenuOpen(false)}
                ...
                <main className="flex-1 overflow-y-auto bg-[#0a0a0b] p-4 md:p-10">
                    <TabsContent value="overview" className="space-y-8 animate-in fade-in-50 slide-in-from-bottom-2 m-0 outline-none">

                        {/* 1. Hero KPI Cards */}
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {[
                                { label: 'CPU Load', value: '4.2%', sub: '2 cores active', icon: Cpu, color: 'text-blue-400', bg: 'bg-blue-500/10' },
                                { label: 'Heap Memory', value: '1.8GB', sub: '4GB limit', icon: Database, color: 'text-violet-400', bg: 'bg-violet-500/10' },
                                { label: 'Avg Latency', value: '124ms', sub: 'Last 1h', icon: Zap, color: 'text-amber-400', bg: 'bg-amber-500/10' },
                                { label: 'Requests', value: '12.4k', sub: 'Since restart', icon: Globe, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
                            ].map((stat, i) => (
                                <Card key={i} className="bg-white/[0.03] border-white/10 overflow-hidden relative group hover:border-white/20 transition-all">
                                    <div className={`absolute -right-6 -top-6 w-24 h-24 ${stat.bg} blur-3xl rounded-full opacity-50 group-hover:opacity-100 transition-opacity`}></div>
                                        ...
                                        <div className="flex justify-between items-start mb-4">
                                            ...
                                            <div className="flex gap-0.5 items-end h-6">
                                                {[40, 60, 45, 70, 50].map((h, idx) => (
                                                    <div key={idx} style={{ height: `${h}%` }} className="w-1 bg-white/20 rounded-full"></div>
                                                ))}
                                            </div>
                                        </div>
                                        <div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="space-y-6">
                                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Control Center</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    >
                                        <CardContent className="p-5 flex flex-col gap-3">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-amber-100 group-hover:text-amber-50">Test Runner</h4>
                                    ...
                                    >
                                        <CardContent className="p-5 flex flex-col gap-3">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-emerald-100 group-hover:text-emerald-50">App Home</h4>
                                    ...
                                    >
                                        <CardContent className="p-5 flex flex-col gap-3">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-indigo-100 group-hover:text-indigo-50">Regen Prompts</h4>
                                ...
                                <div className="p-4 rounded-xl border border-violet-500/20 bg-gradient-to-br from-violet-600/10 to-transparent">
                                    ...
                                    <p className="text-xs text-gray-400 leading-relaxed">
                                        System is capturing all network traffic and AI chain-of-thought. Exception breakpoints are enabled.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Infrastructure Health</h3>
                                    <Button variant="outline" size="sm" className="h-7 text-xs bg-white/5 border-white/10 text-gray-400 hover:text-white" onClick={handleRefresh}>
                                ...
                                <Card className="bg-[#0f0f11] border-white/10">
                                    <div className="divide-y divide-white/5">
                                        {[
                                            { name: 'Supabase Database', status: 'Operational', latency: '42ms', uptime: '100%', icon: Database, color: 'text-green-400' },
                                            { name: 'Supabase Auth', status: 'Operational', latency: '28ms', uptime: '100%', icon: ShieldCheck, color: 'text-green-400' },
                                            { name: 'Gemini 2.0 Flash', status: 'Operational', latency: '1.2s', uptime: '99.9%', icon: Zap, color: 'text-amber-400' },
                                            { name: 'Vercel Edge Runtime', status: 'Optimal', latency: '12ms', uptime: '100%', icon: Server, color: 'text-blue-400' },
                                        ].map((service, i) => (
                                    ...
                                    <div className="p-3 bg-white/[0.02] border-t border-white/5 text-center">
                                        <p className="text-[10px] text-gray-500">Last health check performed at {currentTime.toLocaleTimeString()}</p>
                                    </div>
                                </Card>
                            </div>
                        </div>
                    ...
                    <TabsContent value="api" className="space-y-6 m-0 outline-none animate-in fade-in-50">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    value={sortConfig.key === 'default' ? 'default' : `${sortConfig.key}-${sortConfig.direction}`}
                                    onChange={(e) => {
                        ...
                        <div className="space-y-6">
                            {sortedGroups.map((group, groupIndex) => (
                                <Card key={groupIndex} className="bg-[#0f0f11] border-white/10 text-white overflow-hidden">
                                    <CardHeader className="border-b border-white/5 bg-white/[0.02] py-4 px-6">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <CardTitle className="text-base font-medium text-violet-100">{group.title}</CardTitle>
                                                <CardDescription className="text-xs text-gray-500 mt-1">{group.description}</CardDescription>
                                            </div>
                                        </div>
                                    </CardHeader>
                                    <CardContent className="p-0">
                                        <div className="divide-y divide-white/5">
                                            {group.endpoints.map((endpoint, i) => (
                                                <div key={i} className="flex items-center justify-between p-4 px-6 hover:bg-white/[0.02] transition-colors group">
                                                    <div className="flex items-center gap-4">
                                                        <div className="flex items-center gap-3 w-64">
                                                            <span className={`text-[10px] font-bold px-2 py-1 rounded w-16 text-center border ${endpoint.method === 'GET' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-violet-500/10 text-violet-400 border-violet-500/20'}`}>
                                                                {endpoint.method}
                                                            </span>
                                                            <p className="font-mono text-sm text-gray-300 group-hover:text-white transition-colors">{endpoint.path}</p>
                                                        </div>
                                                        <span className="text-[10px] text-gray-500 bg-white/5 px-2 py-0.5 rounded">{endpoint.type}</span>
                                                    </div>
                                                    <div className="flex items-center gap-8">
                                                        ...
                                                        <div className="text-right w-20">
                                                            <p className={`text-xs font-mono ${getLatencyValue(endpoint.latency) > 1000 ? 'text-amber-400' : 'text-gray-400'}`}>{endpoint.latency}</p>
                                                        </div>
                                    ...
                                    {logsLoading && systemLogs.length === 0 ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-500">
                                            ...
                                            <p className="text-xs text-gray-600 max-w-xs text-center">
                                                Logs will appear here as you interact with the application. Try running a diagnosis or uploading an image.
                                            </p>
                                        </div>
                                    ) : (
                                        <>
                                            <p className="text-gray-600 mb-2">--- System Logs ({systemLogs.length} entries) ---</p>
                                            {systemLogs.map((log) => {
                    ...
                    <TabsContent value="settings" className="space-y-6 m-0 outline-none animate-in fade-in-50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <Card className="bg-white/[0.02] border-white/10 text-white">
                                <CardHeader>
                                    ...
                                    <CardDescription>Runtime configuration - Click the test button to verify connectivity</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-3">
                                    {[
                                        { key: 'NEXT_PUBLIC_SUPABASE_URL', value: 'https://***.supabase.co', testable: true },
                                        ...
                                        <div key={i} className="flex flex-col gap-2 p-3 rounded-lg bg-black/20 border border-white/5">
                                            <div className="flex items-center justify-between">
                                                <span className="text-[10px] text-violet-400 font-bold tracking-wider">{env.key}</span>
                                                {env.testable && (
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        className={`h-6 px-2 text-[10px] gap-1 rounded-full transition-all ${envTestStatus[env.key] === 'success'
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                {envTestMessage[env.key] && (
                                                    <span className={`text-[10px] ${envTestStatus[env.key] === 'success' ? 'text-emerald-400' : 'text-red-400'
                                                        }`}>
                                                        {envTestMessage[env.key]}
                                                    </span>
                                                )}
                                ...
                                <div className="divide-y divide-white/5">
                                    {updatesLoading ? (
                                        ...
                                        updates.map((commit, i) => (
                                            <div key={i} className="p-6 hover:bg-white/[0.02] transition-colors flex gap-6 group">
                                                ...
                                                <div className="flex-1 space-y-1">
                                                    ...
                                                    <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                                                        ...
                                                        <span>•</span>
                                                        <span>{new Date(commit.date).toLocaleDateString(undefined, {
                                                            month: 'short',
                                                            day: 'numeric',
                                                            hour: '2-digit',
                                                            minute: '2-digit'
