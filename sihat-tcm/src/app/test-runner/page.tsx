...
import { CheckCircle, XCircle, Loader2, AlertTriangle, Terminal, Play, Copy, ChevronRight, ChevronDown, RefreshCw, Check, Filter, X, Search } from 'lucide-react'
    ...
    const executeTest = async (testId: string) => {
        switch (testId) {
            ...
            case 'gender_options_validation': {
                ...
                testGenders.forEach(gender => {
                    ...
                    if (isValid !== expectedValid) {
                        throw new Error(`Gender validation failed for '${gender}': expected ${expectedValid}, got ${isValid}`);
                    }
                });
                break;
            }
            ...
            case 'symptoms_text_validation': {
                ...
                symptomsToTest.forEach(({ input, expected }) => {
                    ...
                    if (isInputValid !== expected) {
                        throw new Error(`Symptoms validation failed for "${input.slice(0, 20)}...": expected ${expected}, got ${isInputValid}`);
                    }
                });
                break;
            }
            ...
            case 'face_complexion_analysis': {
                ...
                const result = await response.json();
                if (!result.observation) throw new Error('Face analysis response missing observation field');

                // Content-based check for TCM complexion indicators
                const complexionTerms = ['complexion', 'color', 'face', 'luster', 'bright', 'dull', 'pale', 'red', 'yellow', 'white', 'black', 'qi', 'blood'];
            ...
            case 'audio_file_format_support': {
                ...
                const audioFormats = [
                    { format: 'webm', mimeType: 'audio/webm', base64: 'UklGRi4AAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQAAAAA=' },
                    { format: 'wav', mimeType: 'audio/wav', base64: 'UklGRiQAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQAAAAA=' },
                    { format: 'mp3', mimeType: 'audio/mp3', base64: '//uQxAAAAAAAAAAAAAAAAAAAAAAASW5mbwAAAA8AAAACAAABhgA=' },
                    { format: 'mpeg', mimeType: 'audio/mpeg', base64: '//uQxAAAAAAAAAAAAAAAAAAAAAAASW5mbwAAAA8AAAACAAABhgA=' }
                ];
                ...
                for (const { format, mimeType, base64 } of audioFormats) {
                    ...
                    if (!response.ok) {
                        throw new Error(`Format ${format} (${mimeType}) not supported - API returned ${response.status}`);
                    }
            ...
            case 'audio_file_format_support': {
                ...
                const formats = [
                    ...
                    { format: 'wav', mime: 'audio/wav', data: 'data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQAAAAA=' }
                ];
            ...
            case 'audio_quality_indicators': {
                ...
                if (result.voice_quality_analysis) {
                    ...
                    if (!vqa.observation && !vqa.severity) {
                        throw new Error('voice_quality_analysis missing required fields (observation or severity)');
                    }
            ...
            case 'smart_connect_blood_pressure': {
                ...
                MOCK_PROFILES.forEach(profile => {
                    ...
                    if (typeof bp !== 'string') throw new Error(`${profile.id}: bloodPressure should be a string`);

                    const bpParts = bp.split('/');
                    if (bpParts.length !== 2) {
                        throw new Error(`${profile.id}: bloodPressure format invalid, expected "systolic/diastolic", got "${bp}"`);
                    }
                    ...
                    if (isNaN(systolic) || systolic < 70 || systolic > 200) {
                        throw new Error(`${profile.id}: systolic pressure ${systolic} out of valid range (70-200)`);
                    }

                    // Validate diastolic range (typical: 40-120 mmHg)
                    if (isNaN(diastolic) || diastolic < 40 || diastolic > 120) {
                        throw new Error(`${profile.id}: diastolic pressure ${diastolic} out of valid range (40-120)`);
                    }

                    // Validate that systolic > diastolic
                    if (systolic <= diastolic) {
                        throw new Error(`${profile.id}: systolic (${systolic}) must be greater than diastolic (${diastolic})`);
                    }
                });
                break;
            }
            ...
            case 'health_data_provider_validation': {
                ...
                MOCK_PROFILES.forEach(profile => {
                    ...
                    requiredFields.forEach(field => {
                        ...
                        if (typeof value !== field.type) {
                            throw new Error(`${profile.id}: healthData.${field.name} should be ${field.type}, got ${typeof value}`);
                        }

                        // Check ranges for numeric fields
                        if (field.type === 'number') {
                            if (value < field.min || value > field.max) {
                                throw new Error(`${profile.id}: healthData.${field.name} value ${value} out of range (${field.min}-${field.max})`);
                            }
                        }
                    });
                    ...
                    if (Math.abs(hd.heartRate - pulseRate) > 5) {
                        console.warn(`${profile.id}: heartRate (${hd.heartRate}) differs significantly from pulseRate (${pulseRate})`);
                    }
                });
                ...
                if (uniqueProviders.size < 2) {
                    throw new Error(`Expected multiple health data providers, found only: ${Array.from(uniqueProviders).join(', ')}`);
                }
            ...
            case 'report_diagnosis_structure': {
                ...
                MOCK_PROFILES.forEach(profile => {
                    ...
                    if (typeof report.diagnosis === 'object') {
                        ...
                        throw new Error(`${profile.id}: diagnosis should be either a string or DiagnosisPattern object`);
                    }
                });
                break;
            }
                ...
                MOCK_PROFILES.forEach(profile => {
                    ...
                    if (rec.food_therapy) {
                        ...
                        if (ft.beneficial && !Array.isArray(ft.beneficial)) {
                            throw new Error(`${profile.id}: recommendations.food_therapy.beneficial should be an array`);
                        }
                        if (ft.recipes && !Array.isArray(ft.recipes)) {
                            throw new Error(`${profile.id}: recommendations.food_therapy.recipes should be an array`);
                        }
                        if (ft.avoid && !Array.isArray(ft.avoid)) {
                            throw new Error(`${profile.id}: recommendations.food_therapy.avoid should be an array`);
                        }
                    }
                });
                break;
            ...
            case 'report_constitution_analysis': {
                ...
                MOCK_PROFILES.forEach(profile => {
                    ...
                    if (typeof report.constitution === 'object') {
                        ...
                        if (constitution.description !== undefined && typeof constitution.description !== 'string') {
                            throw new Error(`${profile.id}: constitution.description should be a string if present`);
                        }
                    } else if (typeof report.constitution !== 'string') {
                        throw new Error(`${profile.id}: constitution should be either a string or Constitution object`);
                    } else {
            ...
            case 'mock_report_all_fields': {
                MOCK_PROFILES.forEach(profile => {
                    const report = generateMockReport(profile.data);
                    const required = ['diagnosis', 'recommendations', 'constitution', 'analysis', 'patient_summary'];
            ...
            case 'data_type_validation': {
                MOCK_PROFILES.forEach(profile => {
                    ...
                    if (typeof d.basic_info.name !== 'string') throw new Error(`${profile.id}: basic_info.name should be string`);
                    if (typeof d.basic_info.age !== 'number') throw new Error(`${profile.id}: basic_info.age should be number`);
                    if (typeof d.basic_info.gender !== 'string') throw new Error(`${profile.id}: basic_info.gender should be string`);
                    if (!Array.isArray(d.wen_inquiry.chat)) throw new Error(`${profile.id}: wen_inquiry.chat should be array`);
                    if (typeof d.wang_tongue.observation !== 'string') throw new Error(`${profile.id}: wang_tongue.observation should be string`);
                    if (!Array.isArray(d.wang_tongue.potential_issues)) throw new Error(`${profile.id}: wang_tongue.potential_issues should be array`);
                });
                break;
            }
            ...
            case 'api_response_times': {
                const timeoutLimits = [
                    { endpoint: '/api/health', method: 'GET', body: null, maxTime: 10000, name: 'Health Check' },
                    {
                        endpoint: '/api/chat',
                        method: 'POST',
                        body: JSON.stringify({
                ...
                for (const { endpoint, method, body, maxTime, name } of timeoutLimits) {
                    ...
                    try {
                        ...
                        if (duration > maxTime) {
                            throw new Error(`${name} (${endpoint}) took ${duration.toFixed(0)}ms, exceeds ${maxTime}ms limit`);
                        }
            ...
            case 'report_multilingual_support': {
                ...
                const supportedLanguages: Array<{ code: string, name: string, contentCheck: RegExp }> = [
                    { code: 'en', name: 'English', contentCheck: /\b(diagnosis|recommendation|patient|pattern|deficiency)\b/i },
                ...
                for (const lang of supportedLanguages) {
                    ...
                    if (!response.ok) {
                        throw new Error(`Report generation failed for ${lang.name} (${lang.code}): API returned ${response.status}`);
                    }
                    ...
                    if (!fullText || fullText.length < 100) {
                        throw new Error(`Insufficient content received for ${lang.name} (${lang.code}): only ${fullText.length} characters`);
                    }

                    // Verify content contains expected language patterns
                    if (!lang.contentCheck.test(fullText)) {
                        throw new Error(`Response for ${lang.name} (${lang.code}) does not contain expected language patterns. Received ${fullText.length} chars but pattern check failed.`);
                    }
                }
            ...
            case 'rate_limiting_behavior': {
                ...
                if (!allSucceeded) {
                    const failedStatuses = responses.filter(r => !r.ok && r.status !== 500).map(r => r.status);
...
Please help me troubleshoot this issue. Analyze the code related to '${test.id}' and suggest a fix.`
    ...
    const hasActiveFilters = statusFilter !== 'all' || categoryFilter !== 'all' || criticalOnly || searchQuery.trim() !== ''
                    ...
                    <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                ...
                                <div>
                                    ...
                                    <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as TestStatus | 'all')}>
                                        <SelectTrigger id="status-filter" className="bg-slate-950 border-slate-700 text-slate-200">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                ...
                                <div>
                                    ...
                                    <Select value={categoryFilter} onValueChange={(value) => setCategoryFilter(value as TestCategory | 'all')}>
                                        <SelectTrigger id="category-filter" className="bg-slate-950 border-slate-700 text-slate-200">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                            ...
                            {hasActiveFilters && (
                                <div className="mt-4 pt-4 border-t border-slate-800">
                                    <p className="text-xs text-slate-400">
                                        Showing <span className="font-semibold text-slate-300">{filteredStats.total}</span> of{' '}
                                        <span className="font-semibold text-slate-300">{testStats.total}</span> tests
                                        {filteredStats.passed > 0 && (
                                            <span className="ml-2 text-emerald-400">
                                                • {filteredStats.passed} passed
                                            </span>
                                        )}
                        ...
                        {categories.map((category) => (
                            <div key={category} className="space-y-3">
                                ...
                                {expandedCategories[category] && groupedTests[category] && groupedTests[category].length > 0 && (
                        ...
                        {filteredTests.length === 0 && hasActiveFilters && (
                            <Card className="bg-slate-900/50 border-slate-800 p-8 text-center">
                                ...
                                <h3 className="text-lg font-semibold text-slate-300 mb-2">No tests match your filters</h3>
                                <p className="text-sm text-slate-500 mb-4">Try adjusting your filter criteria or clear all filters.</p>
                                <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={resetFilters}
