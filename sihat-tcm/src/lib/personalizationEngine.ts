...
export class PersonalizationEngine {
    ...
    async personalizeDietaryRecommendations(
        ...
        for (const recommendation of originalRecommendations) {
            try {
                const personalized_version = await this.adaptDietaryRecommendation(
                    recommendation,
                    factors
                );
     ...
     */
    getOptimalExplanationLevel(factors: PersonalizationFactors): 'basic' | 'intermediate' | 'advanced' {
    ...
    private extractHealthHistory(sessions: DiagnosisSession[]): PersonalizationFactors['health_history'] {
        const previous_diagnoses = sessions.map(s => s.primary_diagnosis).filter(Boolean);
        const constitution_patterns = sessions
            .map(s => s.constitution)
    ...
    private buildDietaryRestrictions(dietary_preferences?: DietaryPreferences): PersonalizationFactors['dietary_restrictions'] {
        return {
    ...
    private adaptLifestyleRecommendation(
        ...
        if (recommendation.toLowerCase().includes('exercise') || 
