...
import { testProperty, assertions, patterns, validators } from "../propertyTestHelpers";
  ...
  describe("Property Test Helpers", () => {
    describe("Assertions", () => {
      ...
      it("should validate medical recommendation safety", () => {
        expect(
          assertions.isSafeMedicalRecommendation("drink green tea", ["peanuts"], ["warfarin"])
        ).toBe(true);

        expect(
          assertions.isSafeMedicalRecommendation("eat peanuts for protein", ["peanuts"], [])
        ).toBe(false);

        expect(assertions.isSafeMedicalRecommendation("take dangerous overdose", [], [])).toBe(
          false
        );
      });
    });
    ...
    describe("Validators", () => {
      it("should validate health metrics", () => {
        expect(
          validators.validateHealthMetrics({
            heartRate: 70,
