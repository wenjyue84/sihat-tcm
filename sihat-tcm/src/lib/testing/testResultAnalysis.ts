...
export class PropertyTestAnalyzer {
  ...
  analyzeResults(): TestAnalysis {
    ...
    const failurePatterns = this.identifyFailurePatterns(results.filter(r => r.status === 'failed'))
   ...
   */
  private generateRecommendations(failurePatterns: FailurePattern[], successRate: number): string[] {
    const recommendations: string[] = []
    
    if (successRate < 50) {
      recommendations.push('🚨 Low success rate detected. Consider reviewing test logic and data generators.')
    } else if (successRate < 80) {
      recommendations.push('⚠️ Moderate success rate. Some properties may need refinement.')
    } else if (successRate >= 95) {
      recommendations.push('✅ Excellent success rate! Consider adding more edge cases or complex scenarios.')
    }
    
    // Pattern-specific recommendations
    failurePatterns.forEach(pattern => {
      if (pattern.frequency > 3) {
        recommendations.push(`🔍 Frequent ${pattern.pattern} detected (${pattern.frequency} occurrences). ${pattern.suggestedFix}`)
      }
    })
    
    // General recommendations
    if (failurePatterns.length === 0 && successRate === 100) {
      recommendations.push('🎯 All tests passing! Consider increasing test complexity or adding more properties.')
    }
    
    if (failurePatterns.some(p => p.pattern === 'Test Timeout')) {
      recommendations.push('⏱️ Consider reducing PBT_CONFIG.numRuns for complex properties or optimizing test performance.')
    }
