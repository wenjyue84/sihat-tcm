...
export const assertions = {
  ...
  isSafeMedicalRecommendation(
    ...
    const hasDangerousTerms = dangerousTerms.some((term) => lowerRec.includes(term));
   ...
   */
  hasConsistentMultilingualContent(content: { [language: string]: string }): boolean {
    const languages = Object.keys(content);

    // All supported languages should have content
    const requiredLanguages = ["en", "zh", "ms"];
    const hasAllLanguages = requiredLanguages.every(
      (lang) => languages.includes(lang) && content[lang].trim().length > 0
    );
...
export const validators = {
  ...
  validateMedicalData(data: any): boolean {
    return (
      typeof data === "object" && data !== null && typeof data.id === "string" && data.id.length > 0
    );
  },
  ...
  validateHealthMetrics(metrics: {
    ...
    if (metrics.bloodPressure) {
      ...
      if (
        !assertions.isInRange(systolic, 70, 250) ||
