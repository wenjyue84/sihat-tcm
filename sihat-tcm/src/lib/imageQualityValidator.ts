...
export class ImageQualityValidator {
  ...
  private assessLighting(imageData: ImageData): number {
    ...
    if (avgBrightness < 60)
      score *= avgBrightness / 60; // Too dark
  ...
  private assessResolution(imageData: ImageData): number {
    ...
    if (totalPixels >= minRequired)
      return 0.6 + (0.4 * (totalPixels - minRequired)) / (optimalPixels - minRequired);
  ...
  private calculateCompositionGuide(
    imageData: ImageData,
