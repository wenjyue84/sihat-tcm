...
export function getSolarTermsForYear(year: number): SolarTerm[] {
    ...
    const table1 = (lunarStart as unknown as { getJieQiTable: () => Record<string, Solar> }).getJieQiTable();
    const table2 = (lunarEnd as unknown as { getJieQiTable: () => Record<string, Solar> }).getJieQiTable();
