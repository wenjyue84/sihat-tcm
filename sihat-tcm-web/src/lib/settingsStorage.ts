...
export async function getSettingsWithFallback(): Promise<AdminSettings> {
  ...
  if (supabaseAdmin) {
    try {
      const { data, error } = await supabaseAdmin.from("admin_settings").select("*").single();

      if (!error && data) {
        const settings: AdminSettings = {
          ...DEFAULT_SETTINGS,
          geminiApiKey: data.gemini_api_key,
          medicalHistorySummaryPrompt:
            data.medical_history_summary_prompt || DEFAULT_SETTINGS.medicalHistorySummaryPrompt,
 ...
 */
export async function saveSettingsWithFallback(
  ...
  if (supabaseAdmin) {
    try {
      ...
      if (settings.medicalHistorySummaryPrompt !== undefined)
        dbPayload.medical_history_summary_prompt = settings.medicalHistorySummaryPrompt;
      if (settings.dietaryAdvicePrompt !== undefined)
        dbPayload.dietary_advice_prompt = settings.dietaryAdvicePrompt;
      if (settings.backgroundMusicEnabled !== undefined)
        dbPayload.background_music_enabled = settings.backgroundMusicEnabled;
      if (settings.backgroundMusicUrl !== undefined)
        dbPayload.background_music_url = settings.backgroundMusicUrl;
      if (settings.backgroundMusicVolume !== undefined)
        dbPayload.background_music_volume = settings.backgroundMusicVolume;

      const { error } = await supabaseAdmin.from("admin_settings").upsert(dbPayload);
