...
export interface InquiriesService {
    ...
    listByDiagnosisSession(sessionId: string, options?: InquiryListOptions): Promise<ServiceResult<Inquiry[]>>;
...
export function createInquiriesService(supabase: SupabaseInstance): InquiriesService {
    return {
        ...
        async listByUserId(userId: string, options: InquiryListOptions = {}): Promise<ServiceResult<Inquiry[]>> {
        ...
        async listByDiagnosisSession(sessionId: string, options: InquiryListOptions = {}): Promise<ServiceResult<Inquiry[]>> {
            const { page = 1, limit = 50, column = "created_at", ascending = false } = options;
            const offset = (page - 1) * limit;
