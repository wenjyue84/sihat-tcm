...
export interface DiagnosisService {
  ...
  ): Promise<ServiceResult<DiagnosisSession[]>>;
  listByPatientId(
    patientId: string,
    options?: DiagnosisListOptions
  ): Promise<ServiceResult<DiagnosisSession[]>>;
  create(
    data: Omit<SaveDiagnosisInput, "is_guest_session"> & { user_id?: string }
  ): Promise<ServiceResult<DiagnosisSession>>;
  createGuestSession(
    data: SaveDiagnosisInput & { session_token: string }
  ): Promise<ServiceResult<GuestDiagnosisSession>>;
...
export function createDiagnosisService(supabase: SupabaseInstance): DiagnosisService {
  return {
    ...
    async updateNotes(id: string, notes: string): Promise<ServiceResult<void>> {
      const { error } = await supabase.from("diagnosis_sessions").update({ notes }).eq("id", id);
    ...
    async updateFlag(id: string, flag: PatientFlag): Promise<ServiceResult<void>> {
      const { error } = await supabase.from("diagnosis_sessions").update({ flag }).eq("id", id);
    ...
    async delete(id: string): Promise<ServiceResult<void>> {
      const { error } = await supabase.from("diagnosis_sessions").delete().eq("id", id);
      ...
      if (fetchError || !guestSession) {
        return {
          ...
          error: toServiceError(
            fetchError || { message: "Guest session not found or already migrated" }
          ),
        };
      }
