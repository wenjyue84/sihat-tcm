...
export interface PatientsService {
    ...
    updateClinicalSummary(id: string, summary: Patient["clinical_summary"]): Promise<ServiceResult<void>>;
...
export function createPatientsService(supabase: SupabaseInstance): PatientsService {
    return {
        ...
        async list(options: PatientListOptions = {}): Promise<ServiceResult<Patient[]>> {
            const { page = 1, limit = 50, column = "created_at", ascending = false, status, type, createdBy } = options;
        ...
        async listByDoctor(doctorId: string, options: PatientListOptions = {}): Promise<ServiceResult<Patient[]>> {
            return this.list({ ...options, createdBy: doctorId });
        },

        async search(query: string, options: PatientListOptions = {}): Promise<ServiceResult<Patient[]>> {
            ...
            const { data, error } = await supabase
                .from("patients")
                .select("*")
                .or(`first_name.ilike.%${query}%,last_name.ilike.%${query}%,ic_no.ilike.%${query}%,email.ilike.%${query}%`)
                .order(column, { ascending })
                .range(offset, offset + limit - 1);
        ...
        async updateClinicalSummary(id: string, summary: Patient["clinical_summary"]): Promise<ServiceResult<void>> {
            const { error } = await supabase
                .from("patients")
        ...
        async count(options: { status?: PatientStatus; createdBy?: string } = {}): Promise<ServiceResult<number>> {
