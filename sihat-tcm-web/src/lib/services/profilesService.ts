...
export function createProfilesService(supabase: SupabaseInstance): ProfilesService {
    return {
        ...
        async list(options: ProfileListOptions = {}): Promise<ServiceResult<Profile[]>> {
            const { page = 1, limit = 50, column = "created_at", ascending = false, role, isApproved } = options;
        ...
        async listByRole(role: string, options: ProfileListOptions = {}): Promise<ServiceResult<Profile[]>> {
