 ...
 */
export function extractNumber(val: unknown, fallback: number = 0): number {
 ...
 */
export function extractBoolean(val: unknown, fallback: boolean = false): boolean {
  if (val === null || val === undefined) return fallback;
 ...
 */
export function extractArray<T>(val: unknown, fallback: T[] = []): T[] {
  if (val === null || val === undefined) return fallback;
