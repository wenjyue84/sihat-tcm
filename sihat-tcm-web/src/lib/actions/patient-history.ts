...
export async function getHealthTrends(days: number = 30): Promise<ActionResult<HealthTrends>> {
    try {
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to calculate trends" };
    }
}
...
export async function getLastSymptoms(): Promise<ActionResult<string>> {
    try {
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to fetch symptoms" };
    }
}
