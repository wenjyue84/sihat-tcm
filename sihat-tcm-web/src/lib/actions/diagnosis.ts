...
import { sendDiagnosisNotification } from "@/lib/telegram";
...
export async function saveDiagnosis(
  ...
  try {
    ...
    const isGuest = reportData.is_guest_session || !user;

    if (isGuest) {
      // Guest inserts require the admin client to bypass RLS
      // (anon client has no auth session, so RLS blocks the insert)
      const guestDb = supabaseAdmin || supabase;
      ...
      if (error) {
        ...
        if (error.message.includes("does not exist") || error.message.includes("schema cache")) {
          return {
            success: false,
            error:
              "Database migration required. Please run the migration file: supabase/migrations/20250102000001_add_diagnosis_input_data.sql",
          };
        }
      ...
      sendDiagnosisNotification(reportData, data as GuestDiagnosisSession, true).catch(() => {});
    ...
    if (error) {
      ...
      if (
        ...
        return {
          success: false,
          error:
            "Database migration required. Please run the migration file: supabase/migrations/20251226000001_add_doctor_record_fields.sql",
        };
      }
      return { success: false, error: error.message };
    }
    ...
    // Fire-and-forget Telegram notification — never blocks the response
    sendDiagnosisNotification(reportData, data as DiagnosisSession, false).catch(() => {});
...
export async function migrateGuestSessionToUser(
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to migrate guest session",
    };
  }
}
...
export async function seedPatientHistory(): Promise<ActionResult> {
  try {
    ...
    const mockSessions = [
      {
        ...
        full_report: {
          ...
          recommendations: {
            food: ["Black sesame", "Goji berries"],
            herbal_formulas: [{ name: "Liu Wei Di Huang Wan" }],
          },
        },
      },
      {
        ...
        full_report: {
          ...
          recommendations: {
            food: ["Green vegetables", "Citrus"],
            herbal_formulas: [{ name: "Xiao Yao San" }],
          },
        },
      },
      {
        ...
        full_report: {
          ...
          recommendations: {
            food: ["Congee", "Chinese yam"],
            herbal_formulas: [{ name: "Si Jun Zi Tang" }],
          },
        },
      },
    ];
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to seed patient history",
    };
  }
}
