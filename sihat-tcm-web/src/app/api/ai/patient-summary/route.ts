...
export async function POST(req: Request) {
    try {
        ...
        let diagnosisSessions: Array<{ id: string; created_at: string; primary_diagnosis: string | null; full_report: unknown }> = [];
        if (patient.user_id) {
      ...
      ${diagnosisSessions.length === 0 ? "No recent diagnosis history found." :
