...
const SnoreAnalysisSchema = z.object({
  ...
  average_duration_ms: z
    .number()
    .describe("Average duration of each snoring episode in milliseconds"),
  characteristics: z
    .array(z.string())
    .describe(
      'List of observed characteristics of the snoring (e.g., "regular pattern", "intermittent", "loud vibrations")'
    ),
  apnea_risk_indicators: z
    .array(z.string())
    .describe("Any potential sleep apnea risk indicators observed"),
  tcm_analysis: z
    .object({
      pattern: z
        .string()
        .describe(
          "The TCM pattern associated with this type of snoring (e.g., Phlegm-Dampness, Qi Deficiency)"
        ),
      explanation: z.string().describe("How the sound characteristics relate to the TCM pattern"),
      meridians_affected: z
        .array(z.string())
        .describe("Meridians potentially involved (e.g., Spleen, Lung, Stomach)"),
      fatigue_correlation: z
        .string()
        .describe("Analysis of how this snoring pattern might be contributing to daytime fatigue"),
    })
    .describe("Traditional Chinese Medicine perspective on the snoring"),
  tcm_recommendations: z
    .array(
      z.object({
