...
export default function PatientDetailPage() {
  ...
  const handleFlagChange = async (flag: PatientFlag) => {
    if (!patient) return;
    try {
      const { error } = await supabase.from("patients").update({ flag }).eq("id", patient.id);
  ...
  if (loading)
    return <div className="p-12 text-center text-slate-500">Loading patient record...</div>;
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-10">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="ml-auto flex gap-2 items-center">
            <FlagSelect value={patient.flag} onValueChange={handleFlagChange} />
            ...
            <Button className="gap-2 bg-blue-600 hover:bg-blue-700">Start Diagnosis</Button>
          </div>
        </div>
      </header>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-indigo-100 bg-gradient-to-br from-white to-indigo-50/30">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              >
                <RefreshCw
                  className={`w-3.5 h-3.5 mr-1.5 ${generatingSummary ? "animate-spin" : ""}`}
                />
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-sm text-slate-500 text-center py-4">
                {patient.user_id
                  ? "View full history in History tab (coming soon)"
                  : "No linked account history."}
              </div>
