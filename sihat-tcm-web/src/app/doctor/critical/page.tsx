...
import { AlertTriangle, Stethoscope, User, FileText, Loader2, Activity } from "lucide-react";
...
export default function DoctorCriticalPage() {
  ...
  const handleDismiss = async (id: string, currentNotes: string | null) => {
    ...
    try {
      ...
      if (error.code === "42703") {
        alert(
          "This feature requires a database update (missing 'notes' column). Please contact the administrator."
        );
      } else {
  ...
  const criticalInquiries = useMemo(() => {
    return inquiries.filter((inquiry) => {
      ...
      const hasWarnings =
        report.precautions?.warning_signs &&
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5 hidden sm:block">
              High Priority & Emergency Attention Needed
            </p>
          </div>
        </div>
      </header>
          ...
          <div className="mb-6 bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-700 mt-1">
                These patients have been flagged as critical or have reported symptoms requiring
            ...
            <Card className="bg-white/80">
              <CardContent className="py-12 text-center">
                ...
                <p className="text-gray-500 mt-2">
                  There are currently no inquiries flagged as critical or high risk.
                </p>
              </CardContent>
            </Card>
          ) : (
                          ...
                          <div>
                            ...
                            <ul className="list-disc list-inside text-red-700 text-sm bg-red-50 p-3 rounded-lg">
                              {Array.isArray(diagnosis.precautions.warning_signs) ? (
                                diagnosis.precautions.warning_signs.map(
                                  (sign: string, i: number) => <li key={i}>{sign}</li>
                                )
                              ) : (
