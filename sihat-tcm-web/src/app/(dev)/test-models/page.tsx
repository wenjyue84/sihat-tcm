...
export default function TestModelsPage() {
  const [results, setResults] = useState<
    Record<string, { status: "loading" | "success" | "error"; message?: string; time?: number }>
  >({});
          ...
          <Card key={model.id} className="overflow-hidden">
            ...
            {results[model.id]?.status === "success" && (
              <div className="bg-emerald-50 p-4 border-t border-emerald-100 text-sm text-emerald-800">
                <p className="font-semibold text-xs uppercase tracking-wider mb-1 text-emerald-600">
                  Response:
                </p>
