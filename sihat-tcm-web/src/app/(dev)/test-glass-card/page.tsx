...
export default function TestGlassCardPage() {
  ...
  return (
    <div
      className={`min-h-screen transition-colors duration-300 ${isDarkMode ? "dark bg-slate-900" : "bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50"}`}
    >
      {/* Header Controls */}
            ...
            {(["default", "elevated", "subtle", "glow"] as const).map((variant) => (
              <GlassCard key={variant} variant={variant} className="group">
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`p-2 rounded-lg ${variant === "glow" ? "bg-emerald-100 dark:bg-emerald-900/50" : "bg-slate-100 dark:bg-slate-800"}`}
                  >
                    <Zap
                      className={`w-5 h-5 ${variant === "glow" ? "text-emerald-600 dark:text-emerald-400" : "text-slate-600 dark:text-slate-400"}`}
                    />
                  </div>
              ...
              <GlassCard key={intensity} intensity={intensity} variant="elevated">
                ...
                <div className="mt-3 h-1 rounded-full overflow-hidden bg-slate-200 dark:bg-slate-700">
                  <div
                    ...
                    style={{
                      width: intensity === "low" ? "33%" : intensity === "medium" ? "66%" : "100%",
                    }}
                  />
                </div>
              </GlassCard>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <GlassCard glow variant="elevated" intensity="high">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">
                ...
                prop enabled. Hover to see the shimmer effect!
              </p>
            </GlassCard>
