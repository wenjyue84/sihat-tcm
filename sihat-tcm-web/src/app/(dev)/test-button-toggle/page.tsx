...
export default function TestButtonTogglePage() {
  ...
  useEffect(() => {
    const handleFill = () => {
      setEventLog((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] 📥 fill-test-data event received`,
      ]);
    };
    const handleClear = () => {
      setEventLog((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] 🗑️ clear-test-data event received`,
      ]);
    };
  ...
  const handleButtonClick = () => {
    ...
    if (buttonState === "test") {
      ...
      setEventLog((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] 🔄 State: test → filling`,
      ]);
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        setEventLog((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] 🔄 State: filling → clear`,
        ]);
      }, 1500);
      ...
      setEventLog((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] 🔄 State: clear → clearing`,
      ]);
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        setEventLog((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] 🔄 State: clearing → test`,
        ]);
      }, 1500);
    }
  };
    ...
    <div className="min-h-screen bg-slate-900 text-white p-6">
      <div className="max-w-2xl mx-auto space-y-6">
        <h1 className="text-3xl font-bold text-emerald-400">
          🧪 Test: Button Toggle State Machine
        </h1>
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
          ...
          <div className="flex items-center gap-4">
            <div
              className={`px-4 py-2 rounded-lg ${stateInfo[buttonState].color} text-white font-medium`}
            >
        ...
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
          <h2 className="font-semibold text-lg mb-4 text-emerald-400">
            Test Button (Same as Header)
          </h2>
          <div className="flex items-center gap-4">
            <button
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
          ...
          <div className="text-center font-mono text-sm text-slate-300">
            <div className="flex items-center justify-center gap-2 flex-wrap">
              <span className="px-3 py-1 rounded bg-amber-500/20 text-amber-400 border border-amber-500/50">
                test
              </span>
              <span>→</span>
              <span className="px-3 py-1 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/50">
                filling
              </span>
              <span>→</span>
              <span className="px-3 py-1 rounded bg-rose-500/20 text-rose-400 border border-rose-500/50">
                clear
              </span>
              <span>→</span>
              <span className="px-3 py-1 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/50">
