...
export async function generateMealPlan({
  ...
  try {
    ...
    if (preferences) {
      const allergiesText =
        preferences.allergies?.length > 0
          ...
          : "";
      const dietaryTypeText =
        preferences.dietary_type && preferences.dietary_type !== "none"
    ...
    // Prepare the prompt
    let prompt = MEAL_PLANNER_PROMPT.replace("{{constitution}}", constitution)
  ...
  try {
    ...
    let dietaryText = preferences
      ? [
          ...
          preferences.disliked_foods?.length
            ? `Dislikes: ${preferences.disliked_foods.join(", ")}`
            : "",
        ]
          .filter(Boolean)
          .join("\n")
    ...
    const prompt = MEAL_SWAP_PROMPT.replace("{{constitution}}", plan.constitution || "Balanced")
...
export async function checkFoodSuitability({
  ...
  try {
    ...
    const constitution =
      diagnosisReport?.diagnosis?.primary_pattern ||
      diagnosisReport?.constitution?.type ||
      (typeof diagnosisReport?.constitution === "string"
        ? diagnosisReport.constitution
        : "Unknown");
    ...
    let prompt = FOOD_CHECKER_PROMPT.replace("{{constitution}}", constitution)
      .replace("{{syndrome}}", syndrome)
      ...
      .replace(
        "{{image_context}}",
        imageBase64 ? "A photo of the food is provided for visual analysis." : ""
      );
