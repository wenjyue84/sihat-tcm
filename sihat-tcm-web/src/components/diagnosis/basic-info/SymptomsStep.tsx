...
export function SymptomsStep({
      ...
      try {
        ...
        if (report.input_data?.symptoms && Array.isArray(report.input_data.symptoms)) {
          ...
          if (report.input_data.otherSymptoms) {
            const other =
              typeof report.input_data.otherSymptoms === "string"
                ? report.input_data.otherSymptoms
                    .split(",")
                    .map((s: string) => s.trim())
                    .filter(Boolean)
                : Array.isArray(report.input_data.otherSymptoms)
                  ? report.input_data.otherSymptoms
                  : [];
  ...
  const categories: Record<CategoryMode, { id: string; label: string; symptoms: string[] }[]> = {
    simple: [
      {
        id: "common",
        label: t.basicInfo.symptomCategories?.simple?.common || "Common Symptoms",
        symptoms: ["fever", "cough", "headache", "fatigue", "soreThroat", "stomachPain"],
      },
      {
        id: "chronic",
        label: t.basicInfo.symptomCategories?.simple?.chronic || "Chronic / Critical",
        symptoms: [
          "highBloodPressure",
          "diabetes",
          "heartDisease",
    ...
    western: [
      {
        id: "respiratory",
        label: t.basicInfo.symptomCategories?.western?.respiratory || "Respiratory",
        symptoms: ["cough", "shortnessOfBreath", "soreThroat", "pneumonia"],
      },
      {
        id: "digestive",
        label: t.basicInfo.symptomCategories?.western?.digestive || "Digestive",
        symptoms: ["stomachPain"],
      },
      {
        id: "neurological",
        label: t.basicInfo.symptomCategories?.western?.neurological || "Neurological",
        symptoms: ["headache", "stroke", "fatigue"],
      },
      {
        id: "cardiovascular",
        label: t.basicInfo.symptomCategories?.western?.cardiovascular || "Cardiovascular",
        symptoms: ["highBloodPressure", "heartDisease"],
      },
      {
        id: "general",
        label: t.basicInfo.symptomCategories?.western?.general || "General",
        symptoms: ["fever", "diabetes", "cancer"],
      },
    ],
    tcm: [
      {
        id: "wood",
        label: t.basicInfo.symptomCategories?.tcm?.wood || "Wood (Liver)",
        symptoms: ["headache", "stroke", "highBloodPressure"],
      },
      {
        id: "fire",
        label: t.basicInfo.symptomCategories?.tcm?.fire || "Fire (Heart)",
        symptoms: ["heartDisease", "fever"],
      },
      {
        id: "earth",
        label: t.basicInfo.symptomCategories?.tcm?.earth || "Earth (Spleen)",
        symptoms: ["stomachPain", "fatigue", "diabetes"],
      },
      {
        id: "metal",
        label: t.basicInfo.symptomCategories?.tcm?.metal || "Metal (Lung)",
        symptoms: ["cough", "shortnessOfBreath", "soreThroat", "pneumonia"],
      },
      {
        id: "water",
        label: t.basicInfo.symptomCategories?.tcm?.water || "Water (Kidney)",
        symptoms: ["cancer"],
      },
    ],
  };
  ...
  const handleSymptomClick = (symptomKey: string) => {
    ...
    if (selectedSymptoms.includes(symptomKey)) {
      ...
      const cases = [`, ${symptomName}`, `${symptomName}, `, symptomName];
      ...
      if (activeInput === "main") {
        ...
        if (prevMainText) {
          const entry = Object.entries(t.basicInfo.symptoms).find(
            ([_, value]) => value === prevMainText
          );
          if (entry) {
            prevKey = entry[0];
          }
  ...
  const quickSelectionsSection = (
    <div
      ...
      className="mt-4 rounded-xl border border-border/50 bg-card/50 shadow-sm overflow-hidden"
    >
      {/* Header */}
                ...
                <div key={symptomKey} className="group relative">
                  ...
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-popover/95 backdrop-blur-sm border border-border text-popover-foreground text-xs rounded-xl shadow-xl opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-50 hidden md:block scale-95 group-hover:scale-100 origin-bottom">
                    ...
                    <p className="text-muted-foreground leading-relaxed">
                      {t.basicInfo.symptomDescriptions?.[
                        symptomKey as keyof typeof t.basicInfo.symptomDescriptions
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground flex items-center gap-1">
          ...
          Select from below to add to{" "}
          <strong>
            {activeInput === "main"
              ? t.basicInfo.mainConcern || "Main Concern"
              : t.basicInfo.otherSymptoms || "Other Symptoms"}
          </strong>
        </p>
      </div>
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="less-than-1-day">
              {t.basicInfo.durationOptions.lessThan1Day}
            </SelectItem>
