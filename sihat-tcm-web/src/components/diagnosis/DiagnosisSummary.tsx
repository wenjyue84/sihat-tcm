...
export function DiagnosisSummary({ data, onConfirm, onBack }: DiagnosisSummaryProps) {
  ...
  useEffect(() => {
    ...
    if (data.basic_info) {
      ...
      initialSummaries["basic_info"] =
        `${t.basicInfo.fullName}: ${info.name}\n${t.basicInfo.age}: ${info.age}\n${t.basicInfo.gender}: ${info.gender}\n${t.basicInfo.height}: ${info.height}cm\n${t.basicInfo.weight}: ${info.weight}kg\n\n${symptomDisplay}`;
    }

    // Inquiry
    if (data.wen_inquiry) {
      initialSummaries["wen_inquiry"] =
        data.wen_inquiry.summary || t.diagnosisSummary.defaultMessages.inquiryCompleted;
    }

    // Tongue
    if (data.wang_tongue) {
      initialSummaries["wang_tongue"] =
        data.wang_tongue.observation || t.diagnosisSummary.defaultMessages.noObservation;
    }

    // Face
    if (data.wang_face) {
      initialSummaries["wang_face"] =
        data.wang_face.observation || t.diagnosisSummary.defaultMessages.noObservation;
    }

    // Body Part
    if (data.wang_part) {
      initialSummaries["wang_part"] =
        data.wang_part.observation || t.diagnosisSummary.defaultMessages.noObservation;
    }

    // Audio
    if (data.wen_audio) {
      initialSummaries["wen_audio"] =
        data.wen_audio.transcription ||
        data.wen_audio.analysis ||
    ...
    if (data.qie) {
      const qualities =
        data.qie.pulseQualities?.map((q: any) => q.nameEn).join(", ") || t.common.none;
    ...
    if (data.smart_connect) {
      ...
      initialSummaries["smart_connect"] =
        ...
        >
          ...
          >
            <Input
              ...
              onChange={(e) =>
                setAdditionalInfo((prev) => ({ ...prev, emergencyContact: e.target.value }))
              }
              className="w-full h-9 text-sm bg-white"
            />
          </CheckboxOption>
