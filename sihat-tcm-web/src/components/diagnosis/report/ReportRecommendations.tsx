...
export function ReportRecommendations({
    ...
    return (
      <motion.div variants={variants} className="space-y-8 font-serif">
        ...
        {opts.includeDietary !== false && (
          <div>
            <h3 className="text-lg font-bold uppercase mb-4 border-b border-stone-200">{t.report.dietaryRecommendations}</h3>
            <div className="grid md:grid-cols-2 gap-6">
              {getFoodRecommendations().length > 0 && (
                <div>
                  <h4 className="font-bold mb-2">{t.report.foodsToEat}</h4>
                  <ul className="list-disc list-inside space-y-1 text-sm">{getFoodRecommendations().map((f, i) => <li key={i}>{f}</li>)}</ul>
                </div>
              )}
              {getFoodsToAvoid().length > 0 && (
                <div>
                  <h4 className="font-bold mb-2">{t.report.foodsToAvoid}</h4>
                  <ul className="list-disc list-inside space-y-1 text-sm">{getFoodsToAvoid().map((f, i) => <li key={i}>{f}</li>)}</ul>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Diet Card Group */}
      {opts.includeDietary !== false && (
        <motion.div variants={variants} className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-stone-100">
            ...
            {getFoodRecommendations().length > 0 && (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {getFoodRecommendations().map((food, i) => (
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {/* Lifestyle */}
        {opts.includeLifestyle !== false && data.recommendations?.lifestyle && (
          <motion.div variants={variants} className="bg-white rounded-3xl p-6 shadow-sm border border-stone-100">
      ...
      {opts.suggestMedicine && data.recommendations?.herbal_formulas && (
        <motion.div variants={variants} className="bg-white rounded-3xl p-6 shadow-sm border border-stone-100 overflow-hidden">
      ...
      {/* Precautions */}
      {opts.includePrecautions && data.precautions && (
        <motion.div variants={variants} className="bg-rose-50 rounded-3xl p-6 shadow-sm border border-rose-100">
