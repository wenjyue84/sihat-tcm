      ...
      {inputMode === "camera" ? (
        ...
        <div className="bg-gradient-to-br from-slate-50 to-emerald-50 rounded-xl p-6">
          {inputMode === "manual" ? (
            ...
            <TapBpmInput taps={taps} bpm={bpm} handleTap={handleTap} resetTaps={resetTaps} t={t} />
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="text-center">
        <p className="text-slate-600 mb-4">{t.pulse.afterCounting}</p>
        ...
        {manualBpm && bpmValue > 0 && (
          <div className="mt-4">
            <EcgAnimation bpm={bpmValue} isActive={true} height={100} className="w-full" />
          </div>
        )}
        ...
        {manualBpm && (
          <>
            <div
              className={`mt-3 flex items-center justify-center gap-2 ${isAbnormal ? "text-amber-600" : "text-emerald-600"}`}
            >
...
function TapBpmInput({
  ...
  return (
    <div className="text-center space-y-4">
      <p className="text-slate-600">{t.pulse.tapInRhythm}</p>
      <div className="flex flex-col items-center gap-4">
        {/* Large Tap Button - min 44px touch target for mobile */}
        <Button
          variant="outline"
          className={`rounded-full w-28 h-28 active:scale-90 transition-all duration-150 border-2 ${
            taps.length > 0
        ...
        >
          <Heart
            className={`w-10 h-10 ${taps.length > 0 ? "text-rose-500" : "text-emerald-500"}`}
          />
        </Button>
        ...
        {bpm && (
          <>
            ...
            <div className="w-full mt-2">
              <EcgAnimation bpm={bpm} isActive={true} height={100} className="w-full" />
            </div>
          </>
        )}
        {taps.length > 0 && (
  ...
  return (
    <div className="mt-4 p-4 bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl border-2 border-amber-200 animate-in slide-in-from-top-2 duration-300">
      ...
      <p className="text-sm text-amber-700 mb-3">
        {t.pulse?.abnormalBpmTips?.subtitle ||
          "To ensure accurate measurement, please confirm the following:"}
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {tips.map((tip: any, index: number) => (
          <div
            key={index}
            className="flex items-start gap-2 p-2 bg-white/70 rounded-lg border border-amber-100"
          >
      ...
      <p className="mt-3 text-xs text-amber-600 italic">
        {t.pulse?.abnormalBpmTips?.confirmMessage ||
          "If you have ruled out the above factors, you may continue."}
      </p>
