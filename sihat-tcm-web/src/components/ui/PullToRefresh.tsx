...
export function PullToRefresh({
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      const clientY = e.touches[0].clientY;
      const newPullDistance = Math.max(0, Math.min(clientY - stateRef.current.startY, maxPull));
  ...
  const handleTouchEnd = useCallback(async () => {
    ...
    const finalDistance = Math.max(
      0,
      Math.min(stateRef.current.currentY - stateRef.current.startY, maxPull)
    );
        ...
        >
          <div
            className={cn(
              ...
              isPastThreshold || isRefreshing ? "bg-emerald-500" : "bg-emerald-300"
            )}
