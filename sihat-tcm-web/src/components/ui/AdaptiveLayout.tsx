...
import {
  ...
  useDeviceInfo,
  usePlatformClasses,
} from "@/hooks/usePlatformOptimizer";
...
export function AdaptiveLayout({
  ...
  const navigationStyle =
    forceNavigationStyle || (enableAdaptiveNavigation ? adaptations.navigationStyle : "top");
  ...
  const navigationClasses = cn("adaptive-navigation", {
  ...
  const mainClasses = cn("adaptive-main flex-1", {
  ...
  return (
      ...
      style={
        {
          ...
          "--adaptive-spacing":
            spacing === "compact" ? "0.5rem" : spacing === "comfortable" ? "1.5rem" : "1rem",
    ...
    >
      ...
      {showNavigation && navigationContent && (
        <nav className={navigationClasses}>{navigationContent}</nav>
      )}
      ...
      {footer && (
        <footer className="adaptive-footer">
          <div className={containerClasses}>{footer}</div>
        </footer>
      )}
    </div>
  );
...
export function MobileFirstLayout({
  children,
  ...props
}: Omit<AdaptiveLayoutProps, "enableAdaptiveNavigation">) {
  return (
    <AdaptiveLayout {...props} enableAdaptiveNavigation={true} forceNavigationStyle="bottom">
      {children}
    </AdaptiveLayout>
  );
}

export function DesktopFirstLayout({
  children,
  ...props
}: Omit<AdaptiveLayoutProps, "enableAdaptiveNavigation">) {
  return (
    <AdaptiveLayout {...props} enableAdaptiveNavigation={true} forceNavigationStyle="sidebar">
      {children}
    </AdaptiveLayout>
  );
}
...
export function CompactLayout({ children, ...props }: Omit<AdaptiveLayoutProps, "forceSpacing">) {
  return (
    <AdaptiveLayout {...props} forceSpacing="compact" maxWidth="800px">
      {children}
    </AdaptiveLayout>
  );
}

export function ComfortableLayout({
  children,
  ...props
}: Omit<AdaptiveLayoutProps, "forceSpacing">) {
  return (
    <AdaptiveLayout {...props} forceSpacing="comfortable" maxWidth="1400px">
      {children}
    </AdaptiveLayout>
  );
}
...
export function AdaptiveGrid({
  ...
  const adjustedMinWidth =
    deviceInfo.type === "mobile" ? Math.max(minItemWidth * 0.8, 200) : minItemWidth;
