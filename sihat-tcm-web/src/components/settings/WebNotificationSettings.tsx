    ...
    <Card className={className}>
      ...
      <CardContent className="space-y-6">
          ...
          {Object.entries(localPreferences.frequency || {}).map(([freq, enabled]) => (
            <div key={freq} className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">Receive {freq} notifications</p>
              </div>
              <Switch
                checked={enabled as boolean}
                onCheckedChange={(checked) =>
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={syncWithServer} variant="outline" className="flex-1">
            <Settings className="w-4 h-4 mr-2" />
            Sync Devices
          </Button>
        </div>

        <Button onClick={handleSavePreferences} className="w-full" disabled={isSaving}>
          {isSaving ? (
            <>
