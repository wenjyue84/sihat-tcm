...
import { getPatientMedicines, savePatientMedicine, deletePatientMedicine } from "@/lib/actions";
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="med-name">
                    {t.patientDashboard_v1.healthPortfolio.medicines.name}
                  </Label>
                  <Input
                    id="med-name"
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewMedicine({ ...newMedicine, chinese_name: e.target.value })
                    }
                ...
                <div className="space-y-2">
                  <Label htmlFor="med-dosage">
                    {t.patientDashboard_v1.healthPortfolio.medicines.dosage}
                  </Label>
                  <Input
                    id="med-dosage"
                ...
                <div className="space-y-2">
                  <Label htmlFor="med-freq">
                    {t.patientDashboard_v1.healthPortfolio.medicines.frequency}
                  </Label>
                  <Input
                    id="med-freq"
                ...
                <div className="flex items-center space-x-2 pt-8">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setNewMedicine({ ...newMedicine, is_active: !!checked })
                    }
                  />
              ...
              <div className="space-y-2">
                <Label htmlFor="med-notes">
                  {t.patientDashboard_v1.healthPortfolio.medicines.notes}
                </Label>
                <Textarea
                  id="med-notes"
              ...
              <TableBody>
                {medicines.map((med) => (
                  <TableRow key={med.id} className="hover:bg-slate-50/50">
                    ...
                    <TableCell className="text-slate-600 text-xs sm:text-sm hidden sm:table-cell">
                      {med.dosage || "-"}
                    </TableCell>
                    ...
                    <TableCell>
                      <div
                        className={`inline-flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full text-[8px] sm:text-[10px] font-medium uppercase tracking-wider ${med.is_active ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"}`}
                      >
                        <div
                          className={`w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full ${med.is_active ? "bg-emerald-500" : "bg-slate-400"}`}
                        />
            ...
            {medicines.map((med) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <div
                      className={`p-2 rounded-lg ${med.is_active ? "bg-teal-100 text-teal-600" : "bg-slate-200 text-slate-500"}`}
                    >
                ...
                <div className="space-y-1 ml-10">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-slate-500 mt-2">
                    ...
                    {med.stop_date && <span>Stopped: {med.stop_date}</span>}
                    {med.edited_by && <span>Edited by: {med.edited_by}</span>}
                  </div>

                  {med.notes && <p className="text-xs text-slate-500 italic mt-2">{med.notes}</p>}
                  <div className="flex items-center gap-1 mt-3">
                    ...
                    <span
                      className={`text-[10px] uppercase font-bold tracking-wider ${med.is_active ? "text-emerald-600" : "text-slate-400"}`}
                    >
                      {med.is_active
                        ? t.patientDashboard_v1.healthPortfolio.medicines.active
                        : "Stopped"}
