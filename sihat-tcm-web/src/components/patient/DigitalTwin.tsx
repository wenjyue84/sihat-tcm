...
const ORGAN_PATHS = {
  ...
  heart:
    "M108,125 C108,120 102,115 98,120 C92,110 80,115 80,128 C80,140 95,150 100,155 C105,150 115,140 115,128 C115,125 112,125 108,125 Z",
  lungsLeft:
    "M115,110 C125,110 135,120 135,140 C135,160 125,170 120,165 C115,160 115,120 115,110 Z",
  ...
  kidneysLeft:
    "M115,210 C122,210 125,220 122,228 C118,235 112,230 112,220 C112,215 113,210 115,210 Z",
...
export function DigitalTwin({ sessions, loading }: DigitalTwinProps) {
  ...
  const getOrganStatus = (organ: string) => {
    const isCurrentlyAffected = affectedOrgans.some((o: string) =>
      o.toLowerCase().includes(organ.toLowerCase())
    );

    // Special mapping for TCM organs that might be named subtly differently
    const normalizedOrgan = organ.toLowerCase();
    const isLung =
      normalizedOrgan === "lung" &&
      affectedOrgans.some(
        (o: string) => o.toLowerCase().includes("lung") || o.toLowerCase().includes("respiratory")
      );
    const isKidney =
      normalizedOrgan === "kidney" &&
      affectedOrgans.some((o: string) => o.toLowerCase().includes("kidney"));

    if (isCurrentlyAffected || isLung || isKidney) return "affected";

    const wasPreviouslyAffected = Array.from(previouslyAffectedOrgans).some((o: string) =>
      o.toLowerCase().includes(organ.toLowerCase())
    );
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "affected":
        return { fill: "#ef4444", stroke: "#fee2e2", glow: "#ef4444" }; // Red
      case "healing":
        return { fill: "#10b981", stroke: "#d1fae5", glow: "#10b981" }; // Green
      default:
        return {
          fill: "rgba(56, 189, 248, 0.1)",
          stroke: "rgba(56, 189, 248, 0.3)",
          glow: "rgba(56, 189, 248, 0)",
  ...
  return (
    <div className="relative group flex flex-col items-center w-full bg-slate-900 rounded-3xl overflow-hidden shadow-2xl border border-slate-800">
      ...
      <div className="absolute top-4 left-0 w-full flex justify-between px-6 z-20">
        ...
        <div className="text-[10px] text-sky-400/60 font-mono">SYS.V.2.0.4</div>
      </div>
      ...
      <div className="relative w-full aspect-[3/5] max-h-[500px] p-8 flex items-center justify-center">
        ...
        {organs.map((organ) => {
          ...
          return (
            ...
            >
              <div
                className={`flex items-center gap-2 ${status === "affected" ? "text-red-400" : "text-emerald-400"}`}
              >
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {affectedOrgans.map((o: string, i: number) => (
                <div
                  key={i}
                  className="flex items-center gap-1.5 px-2.5 py-1.5 bg-red-500/10 border border-red-500/20 rounded-md"
                >
