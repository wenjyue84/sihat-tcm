...
interface FiveElementsRadarProps {
  ...
  // Diagnosis data to extract organ scores - accepts DiagnosisReport or similar structures
...
export function FiveElementsRadar({
  ...
  // Calculate scores from diagnosis data, history, or constitution
  const scores = useMemo(() => {
    // 1. Priority: Direct props
    if (currentScores) return currentScores;

    // 2. Priority: Diagnosis Data (AI Generated)
    // Check if diagnosisData has the specific five_elements structure
    if (diagnosisData?.analysis?.five_elements?.scores) {
      return diagnosisData.analysis.five_elements.scores;
    }

    // Also check deeper in full_report if passed differently
    if (diagnosisData?.full_report?.analysis?.five_elements?.scores) {
      return diagnosisData.full_report.analysis.five_elements.scores;
    }

    // 3. Priority: Historical Data
    if (historicalScores.length > 0 && timelineIndex >= 0) {
      return historicalScores[timelineIndex];
    }

    // 4. Fallback: Infer from constitution (for demo/new users)
    return inferScoresFromConstitution(constitutionType || "balanced");
  }, [currentScores, historicalScores, timelineIndex, constitutionType, diagnosisData]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-600">
                <div className="flex items-center gap-2">
                  <div
                    ...
                    style={{ backgroundColor: ELEMENT_COLORS.liver }}
                  />
                ...
                <div className="flex items-center gap-2">
                  <div
                    className="w-3 h-3 rounded-full"
                    style={{ backgroundColor: ELEMENT_COLORS.heart }}
                  />
                ...
                <div className="flex items-center gap-2">
                  <div
                    className="w-3 h-3 rounded-full"
                    style={{ backgroundColor: ELEMENT_COLORS.spleen }}
                  />
                ...
                <div className="flex items-center gap-2">
                  <div
                    className="w-3 h-3 rounded-full"
                    style={{ backgroundColor: ELEMENT_COLORS.lung }}
                  />
                ...
                <div className="flex items-center gap-2">
                  <div
                    className="w-3 h-3 rounded-full"
                    style={{ backgroundColor: ELEMENT_COLORS.kidney }}
                  />
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor: isWeak
                        ? ELEMENT_DARK_COLORS[item.elementKey as keyof typeof ELEMENT_DARK_COLORS]
                        : item.color,
                    }}
                  >
                    {item.score}
                    ...
                    <div className="flex-1 h-2 bg-slate-200 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: chartData.find((d) => d.elementKey === selectedElement)
                            ?.color,
                        }}
                      />
                    </div>
...
function getElementDescription(element: string, t: any): string {
  const descriptions: Record<string, string> = {
    liver:
      t.fiveElementsRadar?.descriptions?.liver ||
    ...
    heart:
      t.fiveElementsRadar?.descriptions?.heart ||
    ...
    spleen:
      t.fiveElementsRadar?.descriptions?.spleen ||
    ...
    lung:
      t.fiveElementsRadar?.descriptions?.lung ||
      "The Lung (Metal element) governs Qi and respiration, controls the skin and body hair, and regulates water passages.",
    kidney:
      t.fiveElementsRadar?.descriptions?.kidney ||
...
// Helper function to get recommendations
function getElementRecommendations(
  element: string,
  score: number,
  t: any,
  ...
  // Add specific justification if available (highest priority)
  if (justification) {
    recommendations.push(justification);
  }

  const staticRecs: Record<string, { weak: string[]; normal: string[] }> = {
    liver: {
      weak: [
