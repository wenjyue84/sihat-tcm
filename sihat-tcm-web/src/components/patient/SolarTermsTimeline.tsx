...
import { Calendar, ChevronLeft, ChevronRight, X, Leaf, Sun, Wind, Snowflake } from "lucide-react";
import {
  getCurrentSolarTerm,
...
export function SolarTermsTimeline({ className = "" }: SolarTermsTimelineProps) {
  ...
  useEffect(() => {
    ...
    if (scrollContainerRef.current && currentTerm) {
      const currentIndex = allTerms.findIndex((term) => isCurrentSolarTerm(term));
      if (currentIndex !== -1) {
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-slate-800">{getTermName(currentTerm)}</h3>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-700 font-medium">{getHealthFocus(currentTerm)}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-sm text-slate-700">{getLifestyleAdvice(currentTerm)}</p>
            </div>
          </div>
        </div>
      </Card>
              ...
              >
                <Card
                  className={`relative overflow-hidden transition-all duration-300 ${
                    isCurrent ? "ring-2 ring-offset-2 shadow-xl scale-105" : "hover:shadow-lg"
                  }`}
                  style={{
                ...
                >
                  {/* Season indicator */}
                  <div className={`h-1.5 bg-gradient-to-r ${getSeasonGradient(term.season)}`} />
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-slate-800 truncate">{getTermName(term)}</h4>
            ...
            >
              <div className="p-6 border-b border-slate-200 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    {new Date().getFullYear()} {t.solarTerms?.year || "Year"}
                  </p>
                </div>
                <button
                  onClick={() => setShowCalendar(false)}
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {allTerms.map((term, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`h-2 bg-gradient-to-r ${getSeasonGradient(term.season)}`} />
                        <div className="p-4">
                          <div className="flex items-start gap-3 mb-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <h4 className="font-bold text-slate-800">{getTermName(term)}</h4>
                              <p className="text-sm text-slate-600">
                                {term.date.toLocaleDateString(
                                  language === "zh"
                                    ? "zh-CN"
                          ...
                          <div className="space-y-2 text-sm">
                            <div>
                              ...
                              <p className="text-slate-700 mt-0.5">{getHealthFocus(term)}</p>
                            </div>
