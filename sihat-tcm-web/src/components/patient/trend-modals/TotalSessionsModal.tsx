...
import { Heart, Shield, Sparkles, Calendar, Award, X } from "lucide-react";
...
export function TotalSessionsModal({
  ...
  const getMilestone = (count: number) => {
    if (count >= 20)
      return { title: "Health Champion", icon: "🏆", color: "from-amber-500 to-yellow-500" };
    if (count >= 10)
      return { title: "Wellness Warrior", icon: "⭐", color: "from-emerald-500 to-teal-500" };
    if (count >= 5)
      return { title: "Health Explorer", icon: "🌟", color: "from-blue-500 to-cyan-500" };
    if (count >= 3)
      return { title: "Wellness Seeker", icon: "🌱", color: "from-green-500 to-emerald-500" };
  ...
  const insights = [
    ...
    },
    {
      ...
      description:
        "Regular TCM sessions help identify imbalances early, before they become major health concerns.",
    },
    {
      icon: <Sparkles className="w-5 h-5 text-amber-500" />,
      title: "Building Health Data",
      description:
        "Each session adds valuable data points, helping track patterns and improvements over time.",
    },
  ];
      ...
      <Dialog.Portal>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          <Dialog.Content className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto p-0 outline-none">
            <div
              className={`sticky top-0 bg-gradient-to-r ${milestone.color} text-white p-6 rounded-t-2xl`}
            >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="text-center bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-6 border border-slate-200">
                ...
                <div
                  className={`inline-block px-4 py-2 rounded-full bg-gradient-to-r ${milestone.color} text-white font-semibold text-sm`}
                >
                  {milestone.title}
                </div>
              </div>
              ...
              {nextMilestone && (
                <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
                  ...
                  <p className="text-xs text-emerald-600 mt-2">
                    🎯 Only {nextMilestone - sessionCount} more session
                    {nextMilestone - sessionCount !== 1 ? "s" : ""} to unlock the next achievement!
                  </p>
                </div>
              )}
              ...
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-4 border border-amber-200">
                <p className="text-sm text-amber-800 leading-relaxed">
                  🌿 <strong>TCM Wisdom:</strong> In Traditional Chinese Medicine, consistent health
                  monitoring aligns with the principle of "治未病" (zhì wèi bìng) — treating disease
                  before it arises. Your commitment to regular sessions embodies this ancient
                  wisdom.
                </p>
              </div>
            </div>
