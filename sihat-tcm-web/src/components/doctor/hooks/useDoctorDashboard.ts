...
export function useDoctorDashboard(): UseDoctorDashboardReturn {
        ...
        `)
            ...
            const transformedData: Inquiry[] = sessions.map(session => ({
                ...
                symptoms: Array.isArray(session.symptoms)
                    ...
                    : (session.symptoms || session.full_report?.analysis?.key_findings?.from_inquiry || 'No symptoms recorded'),
    ...
    const hasActiveFilters = Boolean(searchQuery || dateFrom || dateTo || symptomFilter || flagFilter !== 'All');
