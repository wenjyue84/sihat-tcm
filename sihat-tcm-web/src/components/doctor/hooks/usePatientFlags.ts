...
export function usePatientFlags({ useMockData, setInquiries }: UsePatientFlagsProps): UsePatientFlagsReturn {

    const updateProfileFlag = useCallback(async (profileId: string, flag: PatientFlag) => {
    ...
    const updatePatientFlag = useCallback(async (patientId: string, flag: PatientFlag) => {
        ...
        if (useMockData || patientId.startsWith('p') || patientId.startsWith('mock')) {
            setInquiries(prev => prev.map(inq => {
                ...
                if (Array.isArray(patients)) {
                    return { ...inq, patients: patients.map(p => p.id === patientId ? { ...p, flag } : p) };
                } else {
