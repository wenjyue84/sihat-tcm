        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-semibold text-gray-900">{conversation.patient_name}</h3>
              {conversation.flag && conversation.flag !== "Normal" && (
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${
                    conversation.flag === "Critical"
            ...
            <div className={`flex items-center gap-1 text-xs ${status.color}`}>
              ...
              {conversation.patient_email && (
                <span className="text-gray-400 ml-2">• {conversation.patient_email}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              {/* Flag Options */}
              <div className="px-2 py-1.5 text-xs font-medium text-gray-400">Set Priority</div>
              {flagOptions.map((opt) => (
                <DropdownMenuItem
                  key={opt.key || "none"}
                ...
                >
                  <Flag
                    className={`w-4 h-4 mr-2 ${opt.color}`}
                    fill={currentFlag === opt.key ? "currentColor" : "none"}
                  />
                  {opt.label}
                </DropdownMenuItem>
              ))}
