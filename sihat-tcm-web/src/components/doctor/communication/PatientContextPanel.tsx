  ...
  const fetchContextData = async () => {
    ...
    try {
      ...
      const { data: record, error } = await supabase
        .from(table)
        .select(
          "primary_diagnosis, constitution, overall_score, symptoms, medicines, tongue_analysis, face_analysis, inquiry_summary, full_report"
        )
      ...
      setData({
        ...
        constitution:
          typeof report.constitution === "string"
            ? report.constitution
              ...
              <div className="px-4 pb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2.5 mb-3">
                  ...
                  {data.primary_diagnosis &&
                    data.primary_diagnosis !== "Request for Verification" && (
                  ...
                  {data.constitution && (
                    <div className="bg-white rounded-lg p-2.5 border border-green-100 shadow-sm">
                      ...
                      <p className="text-xs font-medium text-green-700 line-clamp-2">
                        {data.constitution}
                      </p>
                    </div>
                  )}
                  ...
                  {data.symptoms && data.symptoms.length > 0 && (
                    <div className="bg-white rounded-lg p-2.5 border border-amber-100 shadow-sm">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {data.symptoms
                          .filter((s) => s !== "Request for Verification")
                          .slice(0, 3)
                          .map((s, i) => (
                            <span
                              key={i}
                  ...
                  {data.tongue_observation && (
                    <div className="bg-white rounded-lg p-2.5 border border-purple-100 shadow-sm">
                      ...
                      <p className="text-[10px] text-purple-700 line-clamp-2">
                        {data.tongue_observation}
                      </p>
                    </div>
                  )}
                  ...
                  {data.face_observation && (
                    <div className="bg-white rounded-lg p-2.5 border border-pink-100 shadow-sm">
                      <div className="flex items-center gap-1.5 mb-1">
                        <span className="text-xs">👤</span>
                        <span className="text-[10px] font-semibold text-pink-800 uppercase tracking-wide">
                          Face
                        </span>
                      </div>
                      <p className="text-[10px] text-pink-700 line-clamp-2">
                        {data.face_observation}
                      </p>
                    </div>
                  )}
