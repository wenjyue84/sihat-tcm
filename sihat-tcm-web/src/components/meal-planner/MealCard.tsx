...
export function MealCard({ mealType, meal, onSwap }: MealCardProps) {
  ...
  const gradientColor =
    mealColors[mealType as keyof typeof mealColors] || "from-slate-400 to-slate-500";
  ...
  return (
    <>
        ...
        >
          ...
          <div className="p-4">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {meal.temperature && (
                <span
                  className={`text-xs px-2 py-1 rounded-full border ${temperatureColors[meal.temperature as keyof typeof temperatureColors] || temperatureColors.Neutral}`}
                >
      ...
      {/* Recipe Modal */}
      {showModal && (
        <RecipeModal meal={meal} mealType={mealType} onClose={() => setShowModal(false)} />
      )}
    </>
  );
}
