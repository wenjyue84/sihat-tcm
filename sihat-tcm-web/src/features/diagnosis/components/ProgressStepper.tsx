...
export function ProgressStepper({
  ...
  const getStepIcon = (id: string) => {
    switch (id) {
      ...
      case "wang_face":
        return <User className="w-4 h-4" />; // Or another face icon
      case "wen_audio":
  ...
  return (
    <div className="w-full mb-6 md:mb-10 px-2">
      ...
      <div className="md:hidden text-center mb-4 flex items-center justify-center gap-2 text-sm">
        ...
        {promptType && <ShowPromptButton promptType={promptType} />}
      </div>
        ...
        <div className="flex-1 flex justify-between items-center relative">
          ...
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                <motion.div
                  className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 shadow-sm ${
                ...
                <span
                  className={`hidden md:block text-xs font-semibold tracking-wide transition-colors duration-300 absolute -bottom-6 w-20 text-center ${
