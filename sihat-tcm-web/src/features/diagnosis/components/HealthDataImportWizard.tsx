...
export function HealthDataImportWizard({
  isOpen,
  onClose,
  onDataImported,
}: HealthDataImportWizardProps) {
  const { t } = useLanguage();
            ...
            {step === "select" && (
              ...
              >
                <p className="text-sm font-medium text-stone-700 mb-2">
                  {t.healthDataImport.selectProvider}
                </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-semibold text-lg text-stone-800">
                        {t.healthDataImport.providers[key]}
                      </span>
            ...
            {step === "connecting" && selectedProvider && (
              ...
              >
                <div className="relative">
                  <div
                    className={`absolute inset-0 rounded-full animate-ping opacity-20 ${providers[selectedProvider].bg.replace("bg-", "bg-")}`}
                  />
                  <div
                    className={`w-24 h-24 rounded-full flex items-center justify-center border-4 bg-white ${providers[selectedProvider].border}`}
                  >
                    {(() => {
                      const Icon = providers[selectedProvider].icon;
                      return (
                        <Icon
                          className={`w-10 h-10 ${providers[selectedProvider].color} animate-pulse`}
                        />
                      );
                    })()}
                  </div>
                ...
                <div className="text-center">
                  <h3 className="text-lg font-bold text-stone-800">
                    {t.healthDataImport.connecting.title}
                  </h3>
                  <p className="text-stone-500">
                    {t.healthDataImport.connecting.description.replace(
                      "{provider}",
                      t.healthDataImport.providers[selectedProvider]
                    )}
                  </p>
                </div>
              </motion.div>
                ...
                <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-emerald-800">
                      {t.healthDataImport.result.success}
                    </h4>
                    <p className="text-xs text-emerald-600">
                      {t.healthDataImport.result.syncedFrom.replace(
                        "{provider}",
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-stone-50 rounded-lg border border-stone-100">
                    ...
                    <div className="text-xl font-bold text-stone-800">
                      {importedData.steps.toLocaleString()}
                    </div>
                  </div>
                  <div className="p-3 bg-stone-50 rounded-lg border border-stone-100">
                    <div className="flex items-center gap-2 mb-1 text-stone-500 text-xs uppercase font-bold tracking-wider">
                  ...
                  <div className="p-3 bg-stone-50 rounded-lg border border-stone-100">
                    ...
                    <div className="text-xl font-bold text-stone-800">
                      {importedData.heartRate}{" "}
                      <span className="text-xs font-normal text-stone-500">
                        {t.healthDataImport.result.bpm}
                      </span>
                    </div>
                  </div>
                  <div className="p-3 bg-stone-50 rounded-lg border border-stone-100">
