...
export function AnalysisLoadingScreen({
  ...
  const DEBUG_STEPS = [
    {
      id: 1,
      label: t.analysisLoading.debugSteps.dataCollected,
      icon: Database,
      description: t.analysisLoading.debugSteps.gatheringInfo,
    },
    {
      id: 2,
      label: t.analysisLoading.debugSteps.preparingAnalysis,
      icon: FileJson,
      description: t.analysisLoading.debugSteps.organizingData,
    },
    {
      id: 3,
      label: t.analysisLoading.debugSteps.connecting,
      icon: Send,
      description: t.analysisLoading.debugSteps.establishingConnection,
    },
    {
      id: 4,
      label: t.analysisLoading.debugSteps.processing,
      icon: Cpu,
      description: t.analysisLoading.debugSteps.preparingConsultation,
    },
    {
      id: 5,
      label: t.analysisLoading.debugSteps.aiAnalysis,
      icon: Sparkles,
      description: t.analysisLoading.debugSteps.generatingInsights,
    },
    {
      id: 6,
      label: t.analysisLoading.debugSteps.receivingResults,
      icon: Loader2,
      description: t.analysisLoading.debugSteps.retrievingAnalysis,
    },
    {
      id: 7,
      label: t.analysisLoading.debugSteps.validating,
      icon: FileJson,
      description: t.analysisLoading.debugSteps.checkingResults,
    },
    {
      id: 8,
      label: t.analysisLoading.debugSteps.renderingReport,
      icon: Layout,
      description: t.analysisLoading.debugSteps.creatingReport,
    },
  ];
  ...
  const patientInfoCards = [
    ...
    },
    {
      ...
      value: basicInfo?.age
        ? `${basicInfo.age} ${t.analysisLoading.yearsOld}`
        : t.analysisLoading.patientInfo.notAvailable,
    ...
    },
    {
      ...
      value: basicInfo?.weight
        ? `${basicInfo.weight} kg${bmi ? ` (BMI: ${bmi})` : ""}`
        : t.analysisLoading.patientInfo.notAvailable,
      ...
      <div className="space-y-8">
        ...
        <div className="text-center space-y-4">
          <div className="relative w-28 h-28 mx-auto">
            ...
            <div
              className="absolute inset-2 border-2 border-teal-400 rounded-full border-b-transparent animate-spin"
          ...
          <div className="space-y-2">
            ...
            <p className="text-stone-500">{t.analysisLoading.subtitle}</p>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {(() => {
                ...
                return (
                  <div
                    className={`flex items-center gap-4 p-4 rounded-xl ${info.bgColor} border border-stone-100`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-lg font-semibold ${info.color} truncate`}>{info.value}</p>
                    </div>
                  </div>
                );
              })()}
            ...
            {patientInfoCards.map((_, idx) => (
              <div
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  idx === currentInfoIndex ? "bg-emerald-500 w-4" : "bg-stone-300"
                }`}
              />
            ))}
          </div>
          ...
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-6 border border-emerald-100 min-h-[120px]">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div>
                  <h3 className="font-bold text-emerald-800 mb-1">{currentFact.title}</h3>
            ...
            {TCM_FACTS.map((_, idx) => (
              <div
                ...
                className={`h-1 rounded-full transition-all duration-500 ${
                  idx === currentFactIndex ? "bg-emerald-500 w-8" : "bg-stone-200 w-4"
                }`}
              />
            ))}
          </div>
        ...
        <div className="bg-gradient-to-br from-stone-50 to-slate-50 rounded-2xl p-5 border border-stone-200 shadow-sm">
          ...
          <div className="relative mb-5">
            ...
            <div
              ...
              style={{
                width: `calc(${Math.min(((currentDebugStep - 1) / 7) * 100, 100)}% - 32px)`,
              }}
            />
          ...
          {elapsedSeconds > 120 && (
            ...
            >
              <AlertCircle className="w-4 h-4 shrink-0" />
              <span>{t.analysisLoading.timeoutWarning}</span>
            </motion.div>
          )}
        </div>
      </div>
