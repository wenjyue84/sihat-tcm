...
async function checkTables() {
  ...
  console.log(
    "patients table:",
    patientsError ? `❌ ${patientsError.message}` : `✓ Exists (${patientsCount} rows)`
  );
  ...
  console.log(
    "profiles table:",
    profilesError ? `❌ ${profilesError.message}` : `✓ Exists (${profilesCount} rows)`
  );
  ...
  const {
    ...
    error: sessionsError,
    count: sessionsCount,
  } = await supabase.from("diagnosis_sessions").select("*", { count: "exact", head: true });

  console.log(
    "diagnosis_sessions table:",
    sessionsError ? `❌ ${sessionsError.message}` : `✓ Exists (${sessionsCount} rows)`
  );
